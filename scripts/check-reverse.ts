...
const CASES: Record<PlatformId, Patch[]> = {
  ...
  fantia: [
    ...
    { terms: ['猫'], sort: 'oldest', ageRating: 'r18' },
    { terms: ['猫'], sort: 'updated', ageRating: 'safe' },
  ],
  google: [
    {
      terms: ['dialect', 'test'],
...
const WILD: Array<{
  ...
  {
    // 旧q内トークン形式(from:/mentions:/#tag)の後方互換読み込み(issue #27で
    // author=/mentions=/tag= の新パラメータへ移行後も、過去に生成されたリンクや
    // 手打ちのクエリを読めることを確認する)
    url: 'https://bsky.app/search?q=%E7%8C%AB%20from%3Aalice.bsky.social%20mentions%3Abob.bsky.social%20%23cat',
    platform: 'bluesky',
    expect: {
      terms: ['猫'],
  ...
  {
    url: 'https://search.bilibili.com/video?keyword=%E7%8C%AB&order=dm&from_source=webtop_search',
    platform: 'bilibili',
    expect: { terms: ['猫'], resultType: 'video', sort: 'danmaku' },
    ignoredMin: 1,
  },
  {
