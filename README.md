...
Sites are chosen so that one set of conditions translates meaningfully across many of them (fandom and creative-work searching is the primary use case); ecosystems whose search conditions would not be shared with other sites (e.g. code search) are out of scope. The technical bar is only "a search URL with a keyword can be opened": even with thin operator support, there is value in not having to retype the query on each site.
