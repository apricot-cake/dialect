...
**なぜ必要か**: コードに演算子を足したとき、チェックリストへの行追加を忘れると、その演算子だけ定期確認をすり抜ける（実際 2026-07-05 に pixiv「人気の目安」と X `filter:links`/`-from:` が漏れていた）。毎回の冒頭で「コードが送るもの」と「チェックリストが見るもの」のドリフトを潰す。

手順:

1. `src/core/platforms/*.ts` を一通り見て、各サイトが実際に送る演算子・URLパラメータを洗い出す。手がかりは2つ:
   - `support` の各エントリで `level !== 'none'` のもの（＝「この概念を扱う」という宣言）
   - `buildUrl` が実際に組み立てる演算子文字列（`filter:links`、`-from:`、`sort=h`、`s_mode=s_tc`、`{N}users入り` など）
2. それを docs/operator-checklist.md の行と突き合わせる。**送っているのに行がない**ものを拾う。
3. 拾ったもののうち、チェックリストの趣旨（「上ほど壊れやすい＝非公式優先」）に照らして **非公式・仕様変更で壊れうるもの** を追加候補にする。公式で安定な演算子（各サイトの `sort=新着/人気` など素直なもの）は、載せると「壊れやすい順」の意図が薄まるので原則見送る（判断に迷えばユーザーに確認）。
4. 追加候補は**フェーズ1で実測してから**行を足す（未検証の期待結果を書かない）。

> 予防側の原則: 演算子を新規追加するコミットには、対応するチェックリスト行も同じコミットで足すのが本来。フェーズ0はその漏れを拾う保険。
