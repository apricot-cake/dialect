...
| Bluesky | `bsky.app/search` | 必要 | 豊富(公式ドキュメントあり。加えて高度な検索オプションの各種パラメータ=送信者/メンション先/ハッシュタグの複数値OR、動画付きのみ、返信フィルタに対応) |
| Instagram | `instagram.com/explore/search` | 必要 | キーワードのみ |
| YouTube | `youtube.com/results` | 不要 | 期間・除外・並び順・スコープ限定ORなど |
...
| Mastodon | `mastodon.social/search` | 必要 | 豊富(除外・完全一致・送信者・期間・返信除外・言語・メディア) |
| FANBOX | `fanbox.cc/tags` | 不要 | 単一タグのみ(全文検索・並び順・除外等は非対応) |
| bilibili | `search.bilibili.com` | 不要 | 豊富(任意期間・タブ切替・弾幕数/収蔵数などの並び順) |
| Fantia | `fantia.jp/posts` | 必要 | カテゴリー・対象読者・タイトル/本文切替・並び順(除外・完全一致は非対応) |
| Google | `google.com/search` | 不要 | 豊富(完全一致モード・任意期間・スコープ限定OR・探すもの) |
