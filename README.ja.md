...
対応サイトは「1つの条件セットが多くのサイトへ意味を保って翻訳できること」を軸に選んでいます(主な用途はファン活動・創作まわりの横断検索)。他サイトと検索条件を共有できない独自の生態系(コード検索など)は対象外です。技術的な追加基準は「キーワード入りの検索URLが開けること」だけで、演算子対応が薄くても各サイトで打ち直さずに済むことに価値があるためです。
