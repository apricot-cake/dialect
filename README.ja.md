...
| GitHub | `github.com/search` | 不要 | 豊富(所有者・組織・言語・ライセンス・スター/フォーク数/サイズ・複数の期間・フォーク包含/公開範囲/アーカイブ/ミラーの絞り込み・トピック・コードのパス・Issueの状態/理由/ラベル/担当者・ユーザーの属性、探すもの(リポジトリ/コード/Issue/プルリクエスト/ディスカッション/ユーザー)) |
| Qiita | `qiita.com/search` | 不要 | タグ・ユーザー指定・タイトルだけ・最低ストック数・期間・並び順・探すもの(記事/質問) |
| Zenn | `zenn.dev/search` | 不要 | 探すもの(記事/本/スクラップ/ユーザー/Publication)・並び順・意味検索 |
