import type { ConceptId, PlatformId, Resolution } from './types.js'
import { pathSegments } from './parse.js'
...
const STRUCTURAL_EXCLUDE: Partial<Record<PlatformId, ReadonlySet<ConceptId>>> = {
  ...
  youtube: new Set(['sortOrder', 'resultType', 'liveOnly', 'fromUser']),
  pixiv: new Set([
    'sortOrder',
    'period',
    'ageRating',
  ...
  niconico: new Set(['sortOrder', 'period', 'genre', 'nicoKind', 'resultType']),
  seiga: new Set(['sortOrder', 'workType']),
  tumblr: new Set(['sortOrder', 'mediaOnly']),
  fantia: new Set(['fantiaAudience', 'fantiaCategory', 'sortOrder', 'titleOnly', 'ageRating']),
  bilibili: new Set(['resultType', 'sortOrder', 'period']),
  misskey: new Set(['fromUser']),
  google: new Set(['language', 'exactMatchMode', 'period', 'safeSearchOff', 'resultType']),
}
