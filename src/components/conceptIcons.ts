import type { LucideIcon } from 'lucide-react'
import {
  Archive,
  ArrowUpDown,
  AtSign,
  BadgeCheck,
  BadgeJapaneseYen,
  Bookmark,
  BookOpen,
  Brain,
  Building2,
  Calendar,
  Captions,
  CircleCheck,
  CircleDot,
  CircleMinus,
  Clock,
  Code,
  Combine,
  Contrast,
  Copy,
  Copyright,
  Cuboid,
  Equal,
  File,
  FileText,
  Flag,
  FolderGit2,
  FolderOpen,
  Gem,
  GitFork,
  Glasses,
  Globe,
  HardDrive,
  Hash,
  Heading,
  Heart,
  Image as ImageIcon,
  Layers,
  LayoutGrid,
  Link,
  Link2,
  Link2Off,
  List,
  MapPin,
  MessagesSquare,
  MessageSquare,
  MessageSquareOff,
  Monitor,
  Orbit,
  Palette,
  Quote,
  Radio,
  Repeat2,
  Reply,
  ScanSearch,
  Scale,
  Search,
  Shapes,
  ShieldAlert,
  ShoppingBag,
  Sparkles,
  Star,
  Tag,
  Unlink,
  User,
  UserCheck,
  Users,
  UserX,
  Video,
  Eye,
} from 'lucide-react'
import type { ConceptId } from '@/core/types'

/**
 * 各条件の先頭に出す性質アイコン。core/conceptDefs はフレームワーク非依存に保ちたい
 * ので、Lucideコンポーネントとの対応表はUI層のここに置く。以前は手書きのSVGパスを
 * 使っていたが字形が歪む(星など)ため、本物のLucideに統一した。
 */
export const CONCEPT_ICONS: Record<ConceptId, LucideIcon> = {
  keywords: Search,
  exactPhrase: Quote,
  keywordsOr: Combine,
  exclude: CircleMinus,
  fromUser: User,
  excludeUser: UserX,
  toUser: Reply,
  mentionsUser: AtSign,
  excludeMentions: AtSign,
  subreddit: MessageSquare,
  domain: Link2,
  excludeDomain: Link2Off,
  linkUrl: Link,
  excludeLinkUrl: Unlink,
  fileType: FileText,
  region: Flag,
  license: Scale,
  exactMatchMode: ScanSearch,
  xList: List,
  hashtag: Hash,
  hashtagOr: Hash,
  excludeHashtag: CircleMinus,
  period: Calendar,
  titleOnly: Heading,
  mediaOnly: ImageIcon,
  videoOnly: Video,
  exactTag: Equal,
  tagTitleCaption: Layers,
  videoLength: Clock,
  workType: Palette,
  resultType: Video,
  language: Globe,
  sortOrder: ArrowUpDown,
  linksOnly: Link,
  verifiedOnly: BadgeCheck,
  excludeReplies: MessageSquareOff,
  repliesOnly: Reply,
  followingOnly: UserCheck,
  liveOnly: Radio,
  fourK: Gem,
  hdOnly: Monitor,
  captionsOnly: Captions,
  creativeCommons: Copyright,
  threeSixty: Orbit,
  vr180: Glasses,
  threeD: Cuboid,
  hdr: Contrast,
  locationOnly: MapPin,
  purchased: ShoppingBag,
  minLikes: Heart,
  minReposts: Repeat2,
  minReplies: MessagesSquare,
  genre: Shapes,
  nicoKind: Building2,
  paidOnly: BadgeJapaneseYen,
  fantiaCategory: LayoutGrid,
  fantiaAudience: Users,
  safeSearchOff: ShieldAlert,
  pixivPopular: Star,
  ageRating: ShieldAlert,
  excludeAi: Sparkles,
  codeLanguage: Code,
  minStars: Star,
  minStocks: Bookmark,
  semanticSearch: Brain,
  minForks: GitFork,
  minSizeKb: HardDrive,
  pushedPeriod: Calendar,
  codeLicense: Scale,
  includeForks: GitFork,
  visibility: Eye,
  topic: Tag,
  searchInName: Heading,
  searchInDescription: FileText,
  searchInReadme: BookOpen,
  archived: Archive,
  mirror: Copy,
  org: Building2,
  fileExtension: FileText,
  filePath: FolderOpen,
  fileName: File,
  issueState: CircleDot,
  issueReason: CircleCheck,
  minComments: MessagesSquare,
  label: Tag,
  assignee: UserCheck,
  updatedPeriod: Calendar,
  fullName: User,
  userLocation: MapPin,
  minFollowers: Users,
  minRepos: FolderGit2,
}
