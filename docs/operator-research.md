...
### pixiv「作品の詳細」実装(2026-07-13、GUI操作、issue #70)

2026-07-07に上記で「実機確認、未実装」と記録していた「作品の詳細」の解像度・縦横比・制作ツールの3項目を実装した。GUI操作(ログイン済み、issue #70)。pixiv「検索オプション」モーダル(ログイン限定機能。`/tags/{q}/artworks`ページの検索オプションボタンから開く)の「作品の詳細」で解像度・縦横比・制作ツールの3項目を実機操作し生成URLを採取、実装した。解像度=`wlt`/`wgt`/`hlt`/`hgt`(4パラメータの組み合わせでlarge/medium/smallの3バケット)、縦横比=`ratio`(0.5/-0.5/0)、制作ツール=`tool`(103種、生の値をそのまま送る)。いずれも`/tags/{q}/artworks`パスに直接付与しても有効(searchエンドポイントへの切替は不要、実機確認済み)。
