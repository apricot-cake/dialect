...
| Bluesky | doc(公式検索ブログ=唯一) + URL叩き(無認証API実測) | ❌ 未 | **あり** | 除外`-語`・`tab=latest`はブラウザで構築URLを開いて確認したのみ。2026-06 追加の構造化フィルタ(`author=`/`media=`等・現在ゲート中)が有効化されたらフォーム採取が要る |
| YouTube | doc(公式ヘルプ) + URL叩き(sp protobuf) + GUI操作 | ✅ 済 | — | 2026-07-07/08 フィルタパネルの「特徴」10項目・タイプ・並び順を実操作採取(未ログイン) |
| note | URL叩き(実測フェッチ) + doc(公式) | ❌ 未 | **あり** | 検索フォーム・`sort=`タブのGUI採取が未。演算子は乏しい(`from:@`のみ公式)が、フォーム操作での網羅確認は未実施 |
| niconico(動画) | doc(snapshot API) + URL叩き(curl件数比較27パターン) + GUI操作(部分) | 🔺 部分 | **あり** | 2026-07-08 ログイン済みでジャンルのフィルタモーダルを操作し17値採取(GUI操作)。並び順・任意期間・動画の長さはURL叩き/`件数比較`・`/tag`ページのブラウザ確認どまりでフォーム採取は未 |
...
**監査候補(表層調査の可能性が残る＝将来 GUI操作で再点検すべき)**: Bluesky(検索フォーム/tab採取が未・将来フィルタ有効化に注意)、note(検索フォーム/sortタブ採取が未)、niconico動画(ジャンル以外=並び順・期間・長さのフォーム採取が未)。この3件は「済み」ではなく、演算子の実在自体はURL叩きで裏取り済みだが CLAUDE.md の「表層で止めない」水準(フォーム/フィルタ操作での網羅採取)には未達。優先度は新規サイト追加より低い。
