# Dialect 演算子・ディープリンク調査

調査日: 2026-07-02(Web調査+一部API実測)。X / Bluesky / YouTube / note の4プラットフォームについて、検索結果ページのURL形式と、URLに埋め込んで動作する検索演算子を調査した結果。演算子スキーマ(翻訳辞書)の設計元データ。

> **実装状況の注記(2026-07-13、#78)**: 本ログに「実測・実装」と記録された演算子のうち、利用文脈適合性テスト(CLAUDE.md「フィールドの採否選定はする」)に落ちる21概念を#78で剪定した(実装を撤回)。**調査記録自体は残す**(演算子の実在は事実であり、撤回＝実装しないだけで「未調査」に戻すわけではない。pixiv解像度/縦横比/制作ツール撤回と同じ扱い)。剪定した演算子: YouTube `sp`の特徴系(4K/HD/HDR/360°/VR180/3D/字幕/CC/場所/購入済み)・動画の長さ、Google `filetype:`/`cr=`/`tbs=sur:`(ライセンス)/`site:`(ユーザー概念としてのdomain。site:フォールバック#42は内部機構として存続)、X `url:`(domain)/`filter:links`/`filter:blue_verified`、Bluesky `domain=`/`excludeDomain=`/`url=`/`excludeUrl=`/`following=`、niconico `l_range`、bilibili `duration=`、mastodon `has:link`、tumblr `postTypes=link`。以下の各節の該当演算子は「実在するが非対応(剪定済み)」と読み替えること。

## 出所タグ台帳(2026-07-09 整理)

このドキュメントは 2026-07-02 の初回調査から積み増してきた経緯記録で、古い節ほど「実測」の一語で **①公式ドキュメント記載 ②URLパラメータを直接組んでの件数比較(curl・URL直叩き) ③実際にサイトの検索フォーム/並び順/詳細検索・フィルタを操作して生成URLを採取** の3つが区別されずに混在している。CLAUDE.md「演算子調査の出所を記録する」ルールに沿い、後から検証できるよう出所タグを定義し、各サイトの現状を台帳化する。**新しい節(2026-07-07 以降)は本文中に出所タグを明記済み**なので、この台帳は主に古い節の読み替えガイドとして使う。

**出所タグの定義**(強い順):

- **GUI操作** — サイトの検索フォーム・並び順・詳細検索/フィルタパネルを実際に操作し、生成されたURLを採取した。「表層で止めない」を満たす最も強い根拠
- **URL叩き** — URLパラメータを直接組んで開き、件数比較や表示結果で動作を確認した(curl・ブラウザで構築URLを開いての確認を含む)。**フォーム/フィルタの採取はしていない**ため、そのサイトが提供する全オプションを網羅したとは限らない
- **doc** — 公式ドキュメント・公式ヘルプに記載がある

**重要な区別**: 古い節の「ブラウザで検証」は、多くが *こちらが組み立てた検証URLを開いて結果を見た* もので、これは **URL叩き** 相当(ブラウザ確認)であって **GUI操作**(フォームから採取)ではない。ここを取り違えて後付けで「GUI操作」と書くと、ルールが防ぎたい「表層調査を済みに偽装する」ことになるため、**GUI操作タグは実際にフォーム/フィルタを操作した記録があるものだけに付ける**。GUI操作タグが無いサイトは「まだ表層調査の可能性がある＝将来の監査対象」として扱う。

### プラットフォーム別 出所台帳

| サイト | 主な出所 | GUI操作(フォーム/フィルタ採取) | 監査候補 | 最終GUI操作・備考 |
|---|---|---|---|---|
| X | doc(igorbrigadir 標準リファレンス) + GUI操作 | ✅ 済 | — | 2026-07-07 `search-advanced` 全項目、2026-07-08 エンゲージメント欄を実操作採取 |
| Bluesky | doc(公式検索ブログ=唯一) + GUI操作 | ✅ 済 | 一部 | 2026-07-09 ログイン済みで検索フォーム(言語ドロップダウン=`&lang=`)・トップ/最新/ユーザータブを実操作採取。**同日2回目の監査で追加発見**: `media=true`は実際にはもうゲート中ではなく機能する(GUIトグルは無いがURLで効く)、`tab=user`(アカウント検索=探す種類「プロフィール」)をタブ操作で確認・実装。**2026-07-11のOR調査で追加発見**(下記「OR対応 GUI裏取り」節参照): ①ログアウト時検索が不可能に変更されていた(既存記録の「ログイン不要」は要更新)②検索ページに「高度な検索オプション」モーダルが新規追加されており未監査の項目が複数ある(要再監査) |
| YouTube | doc(公式ヘルプ) + URL叩き(sp protobuf) + GUI操作 | ✅ 済 | — | 2026-07-07/08 フィルタパネルの「特徴」10項目・タイプ・並び順を実操作採取(未ログイン) |
| note | GUI操作 + doc(公式) | ✅ 済 | — | 2026-07-09 検索フォーム(並び順チップ人気/急上昇/新着=`sort=popular/hot/new`・有料フィルタ=`context=note_for_sale`)を実操作採取(未ログイン)。**同日2回目の監査で追加発見**: 検索結果タブ「クリエイター」(`context=user`)・「マガジン」(`context=magazine`)を探す種類「プロフィール」「シリーズ」として実装。「メンバーシップ」(`context=circle`、人気/新着の並び順を持つ)は未実装のまま残す |
| niconico(動画) | doc(snapshot API) + GUI操作 | ✅ 済 | — | 2026-07-08 ジャンル17値、2026-07-09 ログイン済みで並び順ドロップダウン(9値)・検索フィルタモーダル(動画種別`kind=`/再生時間`l_range=`/任意期間`start=/end=`)を実操作採取。並び順の現行パラメータは`sort=<名前>&order=desc`形式(旧`sort=h`/`sort=f&order=d`も有効なエイリアス)。**同日2回目の監査で追加発見**: 検索結果タブ「ショート」(`/search_shorts/`)・「シリーズ」(`/series_search/`)・「マイリスト」(`/mylist_search/`)・「ユーザー」(`/user_search/`)を発見。ショートは動画本体と同じフィルタ基盤を共有するため実装。シリーズ/マイリスト/ユーザーの専用並び順(登録動画数/作成日/動画追加日時、フォロワー数/投稿動画数/生放送番組数)も同日中にドロップダウンを実操作して採取・実装 |
| niconico静画 | GUI操作 | ✅ 済 | — | 2026-07-07 検索フォーム送信・並び順セレクトを実操作採取(未ログイン) |
| Instagram | URL叩き(未ログイン即リダイレクト実測) + ログイン済み確認 | —(採取対象なし) | — | 2026-07-04/05 ログイン済みで確認、演算子・フィルタが実質ゼロ(Recentタブ廃止)＝採取対象自体が無いため穴なしと確定 |
| Reddit | doc(公式ヘルプ Boolean) + GUI操作 | ✅ 済 | — | 2026-07-07 検索の結果タブ(`type=`)・並び順・期間プリセットを実操作採取(未ログイン) |
| pixiv | doc(公式ヘルプ) + URL叩き + GUI操作 | ✅ 済 | 一部 | 2026-07-07 検索オプションパネルを実操作採取(ログイン済み)。「作品の詳細」の制作ツール/解像度/縦横比は存在を確認済みだが未実装([[field-family-map]]の耐荷重事項) |
| Misskey | doc(router定義) + GUI操作 | ✅ 済 | — | 2026-07-08 ログイン済みで除外・完全一致を実投稿の増減で確定。検索のフィルタ項目は乏しく、持つ演算子は網羅済み |
| はてなブックマーク | URL叩き(curl実測) + doc + GUI操作 | ✅ 済(ログイン済み) | — | 2026-07-07 検索フォームを実操作採取(未ログイン)。2026-07-09、ユーザーがログイン後にログイン済みGUI操作+WebFetchで再点検し解消(下記「はてなブックマークのログイン差分確認」節参照)。セーフサーチ(safe=on/off)を発見・実装、ユーザー指定検索は自分専用機能と確定(引き続き非対応) |
| Twitch | GUI操作(実ブラウザ実測) | ✅ 済(限定的) | — | 2026-07-03 実ブラウザで確認。演算子ゼロ(`type=`のみ)＝採取対象が少なく網羅済み。ログイン概念は検索に影響なし |
| 5ちゃんねる(ff5ch) | URL叩き(curl実測) | —(採取対象が乏しい) | — | 外部スレタイ検索でフォームは`?q=`のみ。`-語`・`@板`をcurlで確定。ログイン機能自体が存在しない(2026-07-08確認) |
| あにまん掲示板 | URL叩き(curl・フォーム定義実測) | —(採取対象が乏しい) | — | フォームの実パラメータは`q`と`flag`のみ。ログイン機能自体が存在しない(2026-07-08確認) |
| tumblr | GUI操作 | ✅ 済 | — | 2026-07-07 詳細検索/フィルタGUIを実操作採取(除外/from:/期間/複数タグ/完全一致/postTypes)、未ログイン |
| Mastodon | doc(公式ガイド) + GUI操作 | ✅ 済 | — | 2026-07-08 ログイン済みで全演算子を実投稿の増減で確認、検索オプションパネルの網羅性も実見(mastodon.social) |
| Pinterest | GUI操作 | ✅ 済 | — | 2026-07-08 フィルタパネル(4択)を実操作採取、除外・引用符が効かないことも実機確認(未ログイン) |
| FANBOX | GUI操作 | ✅ 済 | — | 2026-07-08 検索欄・タグページを実操作で確認、全文検索が存在しないことを確定(未ログイン) |
| bilibili | GUI操作 + URL叩き | ✅ 済 | 一部 | 2026-07-08 タブ・並び順・「更多筛选」を実操作採取(未ログイン)。「分区筛选(ジャンル)」はメンテ中で採取できず(復活したら要採取) |
| Fantia | GUI操作 | ✅ 済 | 一部 | 2026-07-09 ログイン済みで検索フォーム・詳細検索(カテゴリー24種・キーワード検索範囲)・並び順(5値)を実操作採取。検索は要ログイン。クリエイター/無料チケット/商品/コミッション/タグの各タブは投稿検索と性質が異なるため未実装のまま残す |

**監査候補は2026-07-09に全件解消**: 旧・監査候補だった Bluesky / note / niconico動画 の3件を、実際に検索フォーム・並び順・フィルタパネルをGUI操作して採取し直した(詳細は下記「監査候補3件のGUI再点検(2026-07-09)」節)。この再点検で複数の新オプション(niconico 動画種別`kind=`・note 有料`context=note_for_sale`・Bluesky 言語`&lang=`パラメータ化・niconico並び順の追加値)を発見。3件とも「表層で止めない」水準(フォーム/フィルタ操作での網羅採取)を満たした。

**同日2回目の監査(2026-07-09、下記「探す種類の水平展開」節)**: 上の1回目監査で「唯一の残り監視点」としていたBlueskyの`media=`が実は既に機能することを発見。さらに、Bluesky/note/niconicoの検索結果タブ(トップ以外)を深掘りしたところ、いずれも「探す種類(resultType)」に相当する未収録のタブ切り替えが見つかった(Blueskyのユーザータブ、noteのクリエイター/マガジンタブ、niconicoのショート/シリーズ/マイリスト/ユーザータブ)。Reddit/Pinterest/bilibili等で既に採用している「探す種類」概念の横展開として実装。

**注**: 調査のみで未実装のサイト(Threads/TikTok/Facebook、いずれも doc+URL叩き)は Dialect の対応対象外のため台帳から除外した(下記「追加調査: 6プラットフォーム」節に調査記録は残す)。

## 結論サマリ

- **ディープリンク方式は4プラットフォーム全てで成立する**。検索窓で動く演算子は、いずれもURLのクエリパラメータに埋め込んで同一に動作する
- **ログイン必須はXのみ**(2026-07-02時点)。Bluesky / YouTube / note は未ログインで検索結果を閲覧可能。**2026-07-11追記: Blueskyは以後の仕様変更でログアウト時検索が不可能になっている**(下記「OR対応 GUI裏取り」節参照)
- **noteは演算子がほぼ皆無**(公式対応は `from:@noteID` のみ)。除外・完全一致・期間指定は実測で非対応を確認
- 演算子はXが最も豊富だが**全て非公式(undocumented)**。Blueskyは公式ドキュメントがある唯一のプラットフォーム

## 概念×プラットフォーム対応マトリクス

凡例: ◎=完全対応(緑) / ○=動作するが非公式・注意あり(緑〜黄) / △=近似対応(黄) / ✕=非対応(灰)

| 概念 | X | Bluesky | YouTube | note |
|---|---|---|---|---|
| 複数キーワードAND | ◎ スペース区切り | ◎ スペース区切り | ◎ スペース区切り | △ 絞り込みは効くが厳密AND不明 |
| 完全一致 | ◎ `"..."` | ◎ `"..."` | ○ `"..."`(厳密性低下報告あり、verbatim sp=`QgIIAQ%3D%3D`併用可) | ✕ 引用符は無視される(実測) |
| 除外 | ◎ `-語` | ○ `-語`(実測動作・**未文書化**) | ○ `-語`(信頼度中) | ✕(公式ヘルプが非対応と明言) |
| ユーザー指定 | ◎ `from:user` | ◎ `from:handle` | △ 別URL `youtube.com/@handle/search?query=` | ◎ `from:@noteID`(公式機能) |
| 宛先/メンション | ◎ `to:user` | ◎ `mentions:handle` | ✕ | ✕ |
| 期間指定 | ◎ `since:`/`until:`(YYYY-MM-DD) | ◎ `since:`/`until:`(sortAt基準・until排他) | ○ `before:`/`after:`(非公式・中信頼) or △ sp値で今日/今週/今月/今年に丸め | ✕ |
| ハッシュタグ | ◎ `#tag`(要`%23`) | ◎ `#tag`(要`%23`)+ `/hashtag/`ページ | ○ `/hashtag/`ページ(高)、クエリ内`#`(中) | ◎ `/hashtag/`ページ |
| 言語指定 | ◎ `lang:ja` | ◎ `lang:ja` | ✕(hl/glは不安定) | ✕(日本語サービス) |
| メディア付きのみ | ◎ `filter:media`等 | ✕(q演算子なし。`media=true`パラメータは2026-06追加だがフィーチャーゲート中) | —(種別=動画はsp `EgIQAQ%3D%3D`) | ✕ |
| リンク付き | ◎ `filter:links` | △ `domain:`で代用可 | ✕ | ✕ |
| 最低いいね/RP数 | ○ `min_faves:`/`min_retweets:`(中〜高、公式「高度な検索」フォームのエンゲージメント欄に実在。2026-07-08 GUI操作で再確認) | ✕ | ✕ | ✕ |
| リプライ除外 | ◎ `-filter:replies` | ✕(`replies=none`はゲート中) | — | — |
| 認証済みのみ | ○ `filter:blue_verified`(中) | ✕ | — | — |
| ソート/タブ指定 | ◎ `f=`(top/live/media/user) | ○ `tab=`(latest/user/feed、未文書化) | ○ sp値(※ソートは2026-01にUI削除、URL直指定はまだ動作) | ◎ `sort=`(popular/hot/new、記事タブのみ) |
| ログイン要件 | **必須** | 不要 | 不要 | 不要 |

## X

### URLテンプレート

```
https://x.com/search?q=<URLエンコード済みクエリ>&f=live
```

| パラメータ | 意味 |
|---|---|
| `q` | クエリ本体。演算子は全てここに平文で埋め込む(`#`→`%23`必須) |
| `f` | タブ。省略=top(話題・アルゴリズム選別)、`live`=最新(時系列)、`media`、`user` |
| `src` | トラッキング用、省略可 |

- `f=top`(既定)はアルゴリズムが大半を隠すため、**既定は`f=live`にすべき**
- 公式の検索ビルダー: `x.com/search-advanced`(同じq=形式のURLを生成)

### ログイン要件

**必須(2026-07現在、信頼度高)**。2023-06のログインウォール導入後、個別ポストは解除されたが検索ページは残存。未ログインで開くとログイン画面へリダイレクト。→ Dialect側で「Xはログインが必要」の注記が必要。

### 演算子

| 概念 | 構文 | 信頼度 | 備考 |
|---|---|---|---|
| AND | スペース区切り | 高 | ORは大文字`OR`必須 |
| 完全一致 | `"phrase"` | 高 | スペル自動補正の抑止も兼ねる |
| 除外 | `-語` / `-"フレーズ"` | 高 | |
| ユーザー発 | `from:username` | 高 | コロン後にスペース不可 |
| ユーザー宛 | `to:username` | 高 | |
| 期間 | `since:YYYY-MM-DD until:YYYY-MM-DD` | 高 | sinceは含む/untilは含まない。単独では動かず他条件と併用必須。広すぎる期間は間引かれる |
| ハッシュタグ | `#tag` | 高 | **`%23`エンコード必須** |
| 言語 | `lang:ja` | 高 | 短文の言語判定は不正確 |
| メディア | `filter:media` / `filter:images` / `filter:native_video` / `filter:videos` | 高 | |
| リンク | `filter:links` | 高 | メディアURLも含まれる |
| 最低いいね/RP | `min_faves:N` / `min_retweets:N` / `min_replies:N` | 高 | 公式「高度な検索」フォームのエンゲージメント欄(返信/いいね/リポストの最小件数)に3つとも実在。2026-07-08 GUI操作で確認(x.com/search-advancedに入力→生成URLでmin_faves:/min_retweets:を確認) |
| 認証済み | `filter:blue_verified` | 中 | 公式ドキュメントなし |
| リプライ除外 | `-filter:replies` | 高 | `filter:replies`で逆も可 |

### 注意点

- 演算子は全て非公式。特に`blue_verified`(公式フォームから消滅済み)は削除リスク高め → スキーマに信頼度フラグを持たせ定期確認。`min_faves`/`min_retweets`/`min_replies`は公式フォームに実在すると2026-07-08に確認済み(下記追記参照)
- 演算子は1クエリ約22〜23個が上限
- シャドウバン中のアカウントは`from:`でも結果ゼロになりうる(結果空≠構文ミス)

## Bluesky

### URLテンプレート

```
https://bsky.app/search?q=<URLエンコード済みクエリ>[&tab=latest]
```

| パラメータ | 意味 | 状態 |
|---|---|---|
| `q` | クエリ(演算子込み) | 安定 |
| `tab` | `latest`(最新)/`user`/`feed`、省略=Top | 動作するが未文書化(social-appコードで確認) |

- 関連: `bsky.app/hashtag/{tag}`、`bsky.app/profile/{handle}/search?q=`(ユーザー内検索)
- 構造化フィルタパラメータ(`author=`/`media=true`等)が2026-06-26に追加されたが**フィーチャーゲート中のため使用しない**。`q`+q内演算子+`tab=`が最も安全

### ログイン要件

**不要(実測確認済)**。制限: ログアウト表示オプトアウトのアカウントは非表示。`from:me`はログアウト時**黙って無視され全体検索になる**。

### 演算子

| 概念 | 構文 | 信頼度 | 備考 |
|---|---|---|---|
| AND | スペース区切り | 高 | 実測確認 |
| 完全一致 | `"phrase"` | 高 | 公式ドキュメントあり |
| 除外 | `-語` | **中** | **実測で動作するが公式未記載**。仕様変更リスク |
| ユーザー発 | `from:handle.bsky.social` | 高 | 公式。`from:me`は要ログイン |
| メンション | `mentions:handle` / `to:handle` / `@handle` | 高 | 公式。richtext facetのメンションのみ一致 |
| 期間 | `since:YYYY-MM-DD` / `until:YYYY-MM-DD` | 高(構文)/中(挙動) | 公式。sortAt基準・until排他的境界 |
| ハッシュタグ | `#tag` | 高 | **`%23`エンコード必須** |
| 言語 | `lang:ja` | 高 | 公式 |
| ドメイン | `domain:example.com` | 高 | 公式。リンク先ドメインで絞る |
| メディア付き | なし | 高(不在確認) | `filter:media`等は単なる検索語扱い(実測)。`media=true`はゲート中で使用不可 |

### 注意点

- 演算子の公式ドキュメントが存在する唯一のプラットフォーム(bsky.social/about/blog/05-31-2024-search)
- searchPostsV2への移行が進行中(デフォルトが直近投稿ウィンドウになる可能性)→ 将来の挙動変化に注意
- 検索対象はAppViewがインデックスした公開投稿のみ

## YouTube

### URLテンプレート

```
https://www.youtube.com/results?search_query=<URLエンコード済みクエリ>[&sp=<フィルタ値>]
```

| パラメータ | 意味 |
|---|---|
| `search_query` | 検索ボックスと完全等価。演算子埋め込み可 |
| `sp` | フィルタ/並び順(base64エンコードprotobuf)。固定文字列として安全に使える(セッション非依存) |

- チャンネル内検索: `youtube.com/@ハンドル/search?query={キーワード}`(ログイン不要・確実)
- ハッシュタグページ: `youtube.com/hashtag/{tag}`

### ログイン要件

**不要**。EU等では初回にCookie同意画面が挟まる。

### 演算子

| 概念 | 構文 | 信頼度 | 備考 |
|---|---|---|---|
| AND | スペース区切り | 高 | |
| OR | `A | B`(`%7C`) | 高 | Data API公式に記載 |
| 完全一致 | `"phrase"` | 中 | 厳密性低下の報告あり。verbatim sp `QgIIAQ%3D%3D` 併用が緩和策 |
| 除外 | `-語` | 中 | 「効かなくなった」報告も一部あり |
| 期間 | `before:YYYY-MM-DD` / `after:YYYY-MM-DD` | 中〜高 | 非公式だが2026年時点で動作確認あり。任意日付範囲の唯一の手段 |
| チャンネル | クエリ内`@handle`(低)→ 別URLのチャンネル内検索を推奨 | 低〜中 | |
| タイトルのみ | `intitle:` | 中 | 非公式 |
| ハッシュタグ | `/hashtag/`ページ(高)、クエリ内`#`(中) | | |

### sp値(主要なもの)

| フィルタ | sp値 | 信頼度 |
|---|---|---|
| 今日 / 今週 / 今月 / 今年 | `EgIIAg%3D%3D` / `EgIIAw%3D%3D` / `EgIIBA%3D%3D` / `EgIIBQ%3D%3D` | 高 |
| 種別: 動画 / チャンネル / 再生リスト | `EgIQAQ%3D%3D` / `EgIQAg%3D%3D` / `EgIQAw%3D%3D` | 高 |
| 時間: 4分未満 / 4〜20分 / 20分超 | `EgIYAQ%3D%3D` / `EgIYAw%3D%3D` / `EgIYAg%3D%3D` | 高 |
| ソート: アップロード日 / 視聴回数 | `CAI%3D` / `CAM%3D` | アップロード日=**✕効かない**(2026-07-03実測、下記) / 視聴回数=高(実測) |
| ソート+動画の長さの合成 | 視聴回数+短/中/長: `CAMSAhgB`/`CAMSAhgD`/`CAMSAhgC`、アップロード日+同: `CAISAhgB`/`CAISAhgD`/`CAISAhgC` | 視聴回数系=高(実測でソート・長さとも適用)、アップロード日系=長さのみ適用 |
| 完全一致(verbatim) | `QgIIAQ%3D%3D` | 中 |
| 組み合わせ例: 今日+動画のみ | `EgQIAhAB` | 高 |

- **sp値の連結は不可**。複数フィルタは1つのprotobufに合成が必要。任意の組み合わせは実機でフィルタ選択→URLコピーで取得可能
- 2026-01-09の検索フィルタ再編: アップロード日順ソート・評価順・「1時間以内」がUIから削除。Shortsフィルタ追加(sp値未確認)

### 注意点

- 検索結果は非ログインでもCookie/IPでパーソナライズされる(フィルタ自体は同一に適用)
- モバイルではアプリが横取りし、`&sp=`がアプリ内で尊重されるか未確認(PC優先方針なので容認)

## note

### URLテンプレート

```
https://note.com/search?context=note&q=<URLエンコード済みクエリ>&sort=new
```

| パラメータ | 値 | 信頼度 |
|---|---|---|
| `q` | キーワード(`from:@noteID`もここ) | 高(実測) |
| `context` | `note`(記事)/`magazine`/`user`/`circle`(メンバーシップ) | 高(実測) |
| `sort` | `popular`(既定)/`hot`(急上昇)/`new`(新着)。記事タブのみ有効 | 高(実測) |
| `size` | 表示件数(UIは10) | 中 |
| `mode` | `mode=search`(省略可・意味不明) | — |

- ハッシュタグページ: `note.com/hashtag/{タグ}`(#不要・要エンコード)

### ログイン要件

**不要(2026-07-02実測)**。

### 演算子

| 概念 | 対応 | 備考 |
|---|---|---|
| AND | △ | 絞り込み効果はあるが厳密ANDか関連度かは不明(実測) |
| 完全一致 | ✕ | 引用符は無視される(実測: 件数・上位が完全同一) |
| 除外 | ✕ | 公式ヘルプが非対応を明言、「Googleの`site:note.com kw -除外`を推奨」 |
| ユーザー指定 | ◎ `from:@noteID` | **公式機能**(2021-04リリース)。実測確認済 |
| 期間 | ✕ | `since:`等も効かない(検証記事あり) |
| ハッシュタグ | ◎ `/hashtag/`ページ | 厳密一致。検索(`context=hashtag`)は部分一致 |
| 有料/無料 | ✕(URL不可) | UIトグルはあるが`paid_only=true`等のURLパラメータは無効(実測)。クライアントサイド実装 |

### 注意点

- 件数表示は「約10,000件」で頭打ち
- `/hashtag/`ページのソートタブ・有料トグルはクライアントサイド切替でURL固定不可。ソートをURLで指定できるのは`/search`の`sort=`のみ
- 高度な絞り込みのフォールバックとして`site:note.com`のGoogle検索をnote公式自身が案内(→ v2のGoogleフォールバック機能の正当性根拠)

## 実機確認の結果(2026-07-02、ブラウザで検証)

信頼度が中以下だった項目をブラウザで検証した結果:

1. **YouTube `after:`** — ✅ 動作確認。`紫陽花 after:2026-06-25` で全結果が2〜5日前(7/2時点)に収まった
2. **YouTube `before:`** — ✅ 動作確認。`紫陽花 before:2015-01-01` で全結果が12〜17年前
3. **YouTube 引用符** — ❌ 厳密には効かない。存在しないフレーズ `"冷蔵庫で富士山を冷やす"` が通常のキーワード検索のような結果を返した(引用符は実質無視)。UI表示は「効かないことが多い」に強化済み
4. **X `min_faves:`** — ✅ 動作確認(ログイン済みWeb UI)。`台風 min_faves:500 f=live` で確認した全結果が500いいね以上(717/669/828)。※現在は「最低いいね数」「最低リポスト数」としてUI実装済み(min_retweets:の実測は未実施)
5. **Bluesky `-`除外** — ✅ Web UIで動作確認。`天気 -雨` の結果25件に単語「雨」を含む投稿ゼロ。「梅雨」は別トークンとして残る(単語単位の除外)
6. **Bluesky `tab=latest`** — ✅ 動作確認。「最新」タブが選択された状態で開く
7. **note ANDの厳密性** — ❌ 厳密でないことを確認。`冷蔵庫 富士山` で片方しか含まないとみられる記事が多数混在(関連度ベース)。UI表示(一部制限)は現状のままで正しい

未検証のまま残っているもの: X `filter:blue_verified`(UIには「認証済みアカウントだけ」として実装済み・実測は未実施)、X `min_retweets:`(同族のmin_faves:のみ実測済み)、YouTube Shortsフィルタのsp値(未使用)

継続的な動作確認の手順・記録は [operator-checklist.md](operator-checklist.md) に移した。以降の確認結果はそちらに記録する。

### 追加実機確認(2026-07-03、ブラウザで検証)

並び順の三択化に伴い、YouTubeのソートsp値(protobuf計算値)を検証した:

1. **YouTube `CAM%3D`(視聴回数順)** — ✅ 動作確認。`紫陽花` で 593万→499万→455万→393万 と厳密な降順
2. **YouTube `CAMSAhgB`(視聴回数順+4分未満の合成)** — ✅ ソート・長さとも適用を確認。`猫` で 4.8億→5846万→4119万 の降順、かつ単独`CAM%3D`では2位に入る4分超の楽曲動画(2.3億回)が除外された
3. **YouTube `CAI%3D`(アップロード日順)** — ❌ **効かなくなっている**。`紫陽花` で1年前のMVが先頭(関連度と同じ並び)、`台風` でも 10時間前→1時間前→19時間前 と時系列にならない。2026-01のUI削除に続きURL直指定も無効化されたとみられる(上の2026-07-02調査時点の「まだ動作」はWeb情報ベースで、実測で覆った)。`CAISAhgB` 等の合成値も長さフィルタだけが適用される
   → Dialectは「新しい順」でもCAI系を送信し続ける(害はなく、復活すれば効く)が、UI注記で「新しい順は現在効かない」と明示する

## スキーマ設計への示唆

- 演算子定義には **信頼度フラグ**(公式/非公式・実測済/未確認)と **確認日** を持たせる。非公式演算子は予告なく死ぬため、定期的な動作確認を前提にする
- `#`の`%23`エンコード漏れが最頻出のバグ要因(X・Bluesky共通)。シリアライザで必ず`encodeURIComponent`を通すこと
- 期間指定は「任意日付(X/Bluesky/YouTube before・after)」と「プリセット丸め(YouTube sp)」の2形態がある → UIは任意日付で受け、YouTubeはbefore/afterを第一候補、spプリセットを近似フォールバックとする
- Xのみログイン必須 → Xボタンに「ログインが必要です」の常時注記
- ソート/タブ指定(X `f=live`、Bluesky `tab=latest`、note `sort=new`)は概念「新着順」として共通化できる

## 主要ソース

- X: [igorbrigadir/twitter-advanced-search](https://github.com/igorbrigadir/twitter-advanced-search)(事実上の標準リファレンス)、sorsa.io・keep.md・jesusiniesta.es の2026年ガイド、TechCrunch(2023ログインウォール)
- Bluesky: [公式検索ブログ](https://bsky.social/about/blog/05-31-2024-search)(唯一の公式演算子ドキュメント)、atproto lexicons(searchPosts/V2)、social-appソースコード、無認証API実測(2026-07-02)
- YouTube: [公式ヘルプ(検索フィルタ)](https://support.google.com/youtube/answer/111997)、Data API公式、ktsk.xyz(spのprotobuf解析)、SerpApi、ppc.land(2026-01フィルタ再編)、control-panel-for-youtube
- note: note.com実測フェッチ(2026-07-02)、[公式リリース(from:@)](https://note.com/info/n/n216550eab71e)、公式ヘルプ(除外非対応の明言)、karupoimou検索フォーム調査

---

# 追加調査: 6プラットフォーム(2026-07-02)

プロダクト原則「キーワード入りの検索URLが開ければ追加する」に基づく拡張調査。6サイト全てで検索URLの存在を確認し、全て追加した。

## 追加サイトの概要

| | 検索URL | ログイン | 演算子の濃さ |
|---|---|---|---|
| niconico | `nicovideo.jp/search/{query}` | 不要 | **豊富**(AND/完全一致/除外/任意期間/ソート全てURL可・実測27パターン) |
| Reddit | `reddit.com/search/?q=` | 不要(デスクトップ) | **豊富**(AND/NOT/author:/title:は公式仕様。期間はt=プリセットのみ) |
| Threads | `threads.com/search?q=` | **必要** | 薄い(filter=recent と from_author= のみ。q内演算子なし) |
| TikTok | `tiktok.com/search?q=` | 不要(モーダルは閉じられる) | 薄い(引用符が一部効く程度。ソート・期間はURL不可) |
| Instagram | `instagram.com/explore/search/keyword/?q=` | **必要**(即ログインウォール) | ほぼゼロ(タグページは人気投稿のみ・Recent廃止) |
| Facebook | `facebook.com/search/top/?q=` | **必要** | ほぼゼロ(引用符が中信頼で効く。filters=base64は非公開仕様で不採用) |

## シリアライザの要点

- **niconico**: `sort=f&order=d`(新着)/`sort=h`(人気)。デフォルトソートはABテスト依存のため常に明示。任意期間は `start=YYYY-MM-DD&end=YYYY-MM-DD`(実測済)。タグ単独→`/tag/{tag}`、併用時はキーワードに畳み込み。除外`-語`はタグページでも有効。ユーザー指定の演算子は存在しない
- **Reddit**: スペース区切りは緩い一致のため、キーワードを ` AND ` で明示結合して厳密化。除外は公式構文 `NOT (a OR b)`。ユーザーは `author:`。ソートは `sort=new`。期間は `t=day|week|month|year|all` への丸め(sinceから自動選択)。old.reddit.comは2026年8月からログイン必須化予定のため使わない
- **Threads**: `filter=recent`(新着)と `from_author={user}`(2026-04実URL確認)。threads.net→threads.comに301(クエリ保持)。タグ単独→`/tag/{tag}`(ログアウトでも一部表示される唯一の経路)。期間はUIフィルタのみでURL不可
- **TikTok**: `search?q=` のみ。タグ単独→`/tag/{tag}`。除外・期間・ソートは内部APIパラメータでURL非公開。`from:`相当なし
- **Instagram**: `explore/search/keyword/?q=`(スクレイパー各社が常用する事実上の標準)。タグ単独→`/explore/tags/{tag}/`(**人気投稿のみ**)。演算子ゼロ。未ログインは検索・タグとも即リダイレクト(実測)
- **Facebook**: `search/top/?q=` が最安全(`search/posts` はUIから削除済みで将来リスク)。`filters=`(base64の内部仕様)による最新順・日付は キー名の変遷歴があり不採用。引用符はある程度効く(中信頼)

## OR グループの結合規則(2026-07-02、検索ページの件数比較で実測)

「いずれかを含む」行の複数化(GUI での論理の組み立て)に伴い、niconico の OR の結合規則を実測した:

1. **括弧は使えない** — `(猫 OR 犬)` は 11,077件(`猫 OR 犬` は 542,332件)。括弧がリテラル扱いになり検索が壊れる
2. **OR はスペースの AND より強く結合する** — `猫 OR 犬 手芸` = 391件 ≈ `猫 手芸`(253件)+`犬 手芸`(161件)。`猫 OR (犬 手芸)` の解釈なら38万件超になるはずで、実際は `(猫 OR 犬) AND 手芸`
3. **OR 連鎖の並置で複数グループが成立する** — `猫 OR 犬 編み物 OR 手芸` = 412件 ≈ 4つの AND ペアの和(488件)−重複。語順を入れ替えても412件で一致し、文法は「OR 連鎖の AND 結合」で安定

→ niconico のシリアライザは括弧なしの並置(`a OR b c OR d`)で複数 OR グループを表現する。なお snapshot API は同日時点で件数が異常に小さく(`猫`=2件)、検証には使えなかった。

他プラットフォームの複数 OR グループ: X は標準リファレンス(igorbrigadir)に `()` グループ化の記載あり、Reddit は Boolean 演算子+括弧が公式仕様、YouTube は `|` が Data API 公式(括弧は従来実装を踏襲)。

## 参考ソース(抜粋)

- niconico: [スナップショット検索v2 APIガイド](https://site.nicovideo.jp/search-api-docs/snapshot)(AND/OR/-/フレーズの公式仕様)、非ログインcurl実測27パターン(2026-07-02)、ニコニコ大百科(マイナス検索)
- Reddit: [公式ヘルプ Available search features](https://support.reddithelp.com/hc/en-us/articles/19696541895316)(Boolean/フィールド演算子)、Slashdot/Softonic(old.reddit ログイン必須化 2026-07-01発表)
- Threads: TechCrunch(threads.com移行 2025-04)、Raycast quicklinks・自動化プロジェクト(URL形式)、実測(301リダイレクト・ログアウト挙動)
- TikTok: ScrapFly 2026(URL形式・anti-bot)、team5pm(演算子)、tokportal/droid4x(非ログイン閲覧の実態)
- Instagram: axiom.ai/Apify(keyword SERPのURL形式・Cookie必須)、実測(未ログイン即リダイレクト 2026-07-02)、Later/SMT(Recentタブ廃止)
- Facebook: PiunikaWeb(Postsフィルタ削除 2025-09)、Plessas Facebook Matrix(URLテンプレート)、nem.ec(filters= JSON仕様)

# 追加調査: pixiv / Misskey.io(2026-07-03)

## pixiv

### URLテンプレート

```
https://www.pixiv.net/tags/<URLエンコード済みクエリ>/artworks[?order=...&scd=...&ecd=...]
```

| パラメータ | 意味 | 信頼度 |
|---|---|---|
| パス `/tags/{query}/artworks` | イラスト・マンガの検索。クエリに演算子を埋め込む | 高(サイト標準) |
| `s_mode` | 省略=`s_tag`(タグ部分一致)。`s_tag_full`=タグ完全一致、`s_tc`=タイトル・キャプション | 高 |
| `order` | `date_d`=新着(既定)/`date`=古い順/`popular_d`=人気(**プレミアム会員限定**) | 高 |
| `scd` / `ecd` | 期間の開始日/終了日(YYYY-MM-DD)。単体では有効だが `order=date_d` と併用するとエラーページになる(2026-07-04実測) | 中(非公式) |

- 小説は `/tags/{query}/novels` だが Dialect は artworks(イラスト・マンガ)のみ対象
- ユーザー検索は別URL(`/search_user?nick=`)で、作品検索とキーワード併用できないため非対応

### ログイン要件

**不要**。ただし「ログインユーザーに公開」設定の作品とR-18は未ログインでは結果に出ない。原寸表示など閲覧側の機能制限もあるが検索結果一覧は閲覧可能。

### 演算子([公式ヘルプ](https://www.pixiv.help/hc/en-us/articles/235646387)に記載あり)

| 概念 | 構文 | 信頼度 | 備考 |
|---|---|---|---|
| AND | スペース区切り | 高 | タグの部分一致で照合(既定の s_mode=s_tag) |
| OR | `A OR B` / `A (B OR C)` | 高 | **公式ヘルプ記載**。括弧グループ可 |
| 除外 | `-語` | 高 | **公式ヘルプ記載** |
| 完全一致 | ✕ 引用符なし | 高(不在) | タグ完全一致(s_tag_full)はあるが語句の完全一致とは別物 → キーワード近似 |
| ハッシュタグ | タグ語として畳み込み | 高 | pixivのタグ=ハッシュタグそのもの。#を外してクエリへ |
| 期間 | `scd=`/`ecd=` | 中 | 非公式URLパラメータ。2026-07-04実測: 単体は有効(1,243件・期間バッジ表示)。ただし `order=date_d` と併用するとエラー → 新着(既定)時は order を送らない |
| 並び順 | `order=date_d`/`popular_d` | 高/高 | popular_d はプレミアム限定(非会員は新着のまま) |

## Misskey.io

### URLテンプレート

```
https://misskey.io/search?q=<URLエンコード済みクエリ>&type=note[&username=ユーザー名]
```

| パラメータ | 意味 | 信頼度 |
|---|---|---|
| `q` | 検索語。演算子はなく本文の部分一致 | 高(ルーター定義で確認) |
| `type` | `note`(ノート検索)/`user` | 高(同上) |
| `username` / `host` | ユーザーで絞り込み(フロントエンドが users/show で解決) | 中(非公式・ルーター定義とsearch.note.vueの実装で確認) |

- タグページ: `misskey.io/tags/{タグ}`(#不要・要エンコード)
- ルート定義の出典: misskey-dev/misskey `packages/frontend/src/router.definition.ts`(q/userId/username/host/channel/type/origin を search ページの props にマッピング)

### ログイン要件

**必要**。misskey.io のノート検索はログインユーザー向け機能(Misskey本体がロールで検索可否を制御する設計)。タグページは未ログインでも一部閲覧可。

### 演算子

| 概念 | 対応 | 備考 |
|---|---|---|
| AND | △ スペース区切り | 本文の部分一致の組み合わせ。misskey.io公式サポートが「まれな語は結果が出ないことがある」と案内しており安定性は中 |
| OR / 除外 / 完全一致 / 期間 | ✕ | q内の演算子は存在しない → Googleフォールバック対象 |
| ユーザー指定 | ○ `&username=` | URLパラメータ(非公式)。**検索語との併用が必須**(qが空だと検索が実行されない)。※2026-07-04実測: q入りでもURL遷移だけでは検索が自動実行されず、ページ上の「検索」ボタンのクリックが必要(下記の定期確認メモ参照) |
| 除外 | ○ `-語` | 2026-07-08にログイン済みGUI操作で確定(下記「再検証」参照)。実在の投稿を使った前後比較で除外語を含む投稿が結果から消えることを確認 |
| 完全一致 | ✕ **機能しないと確定** | 2026-07-08にログイン済みGUI操作で確定。`"..."`は文字として扱われクエリ自体を壊し、実在の投稿本文をそのまま引用符で括っても0件。他のキーワードと組み合わせると検索全体が0件になる実害も確認したため、Dialectは引用符を送らず語ごとのAND(キーワードと同じ扱い)へ畳み込む |
| ハッシュタグ | ◎ `/tags/`ページ | 単独時。併用時は本文に「#タグ」が含まれる部分一致検索へ畳み込み(ノート本文にタグ文字列が残るため成立) |
| 並び順 | ✕ | 検索結果は新着順固定。URL指定不可 |

### 除外・完全一致の再検証(2026-07-08、ログイン済みGUI操作)

2026-07-04の暫定確認(「猫のみ確認・犬なしの厳密対比は次回」のまま持ち越し)を、既存アカウントでログイン済みの状態で厳密に再検証した。

- **除外(`-語`)**: `misskey.io/search?q=猫&type=note` を実行し、トップ結果に「猫や犬を撫でるときに上から手を出すのは難しくなっていくんかな」という実在の投稿(猫と犬を両方含む)があることを確認。続けて `q=猫 -犬` で再実行すると、この投稿が結果から消え、以降のページを送っても再出現しないことを確認。実在の投稿の増減で確定 ✅
- **完全一致(`"..."`)**: 以下の3段階で機能しないことを確定させた ❌
  1. 単語1つの引用符(`"猫"`)ですら0件(引用符なしの`猫`は多数ヒット)
  2. 実在する投稿本文をそのまま引用符で括った`"そもそもこの猫BSSを好む"`(まめCOMITIA156-と19bの投稿からコピー)ですら0件。投稿が実在するのに一致しない=引用符が文字通り扱われ、クエリ自体が壊れていると確定
  3. 通常のキーワードと組み合わせた`猫 "今日"`も0件。**単独条件だけでなく、他の正常なAND条件まで巻き込んで検索全体を無効化する実害を確認**(単なる「不安定」ではなく「使うと壊れる」)
- **対応**: `src/core/platforms/misskey.ts` の `buildUrl` から完全一致の引用符包みを削除し、`exactPhrases(state).flatMap(words)` で語ごとのAND(keywordsと同じ扱い)へ畳み込むよう修正。support は `exclude: full`・`exactPhrase: partial(note.misskey.exactPhrase)` に更新
- 検証: lint/build/check:operators(0/0/0、misskey PROBESに除外・完全一致を追加)/check:urls(0 broken)

# 追加調査: はてなブックマーク / Mastodon / Twitch / 5ちゃんねる / あにまん掲示板 / ニコニコ静画(2026-07-03)

6サイトすべてWeb調査+未ログインでのHTTP実測(curl、ブラウザUA)。Twitchのみ実ブラウザでも検証。

## 概要

| | 検索URL | ログイン | 演算子の濃さ |
|---|---|---|---|
| はてなブックマーク | `b.hatena.ne.jp/search/text?q=` | 不要 | **豊富**(AND/除外/任意期間/並び順/最低ブクマ数が全てURL可・実測) |
| Mastodon(mstdn.jp) | **キーワード検索URLなし**(タグページのみ) | タグページは不要 | — |
| Twitch | `twitch.tv/search?term=` | 不要 | 薄い(演算子ゼロ。type=で対象切替のみ) |
| 5ちゃんねる | `ff5ch.syoboi.jp/?q=`(スレタイ検索) | 不要 | 中(AND/除外/@板が実測で有効) |
| あにまん掲示板 | `bbs.animanch.com/searchRes/{query}` | 不要 | 薄い(スペースANDのみ。実測) |
| ニコニコ静画 | `seiga.nicovideo.jp/search/{query}` | 不要 | 中〜豊富(AND/除外/OR/並び順が実測で有効) |

## はてなブックマーク

- 検索対象は**パスで指定**: `/search/text`(本文)・`/search/title`(タイトル)・`/search/tag`(タグ)。`target=`パラメータは実効なし
- パラメータ(全て実測・高信頼): `q`(スペース区切りAND、`-語`除外)、`sort=recent|popular`(既定popular)、`date_begin`/`date_end`(YYYY-MM-DD)、`users=N`(最低ブクマ数。UI選択肢は1/3/50/100/500)、`page=N`
- 引用符 `"..."` は絞り込み効果を実測したが公式ヘルプに記載なし(中信頼)
- **落とし穴**: 期間を指定しないと、はてブ側の標準で「直近5年・users=3」に絞られる(date_begin明示で全期間になる)。サイト上で普通に検索しても同じ挙動のため、Dialectは site の既定に任せる
- ユーザーのブックマーク内キーワード検索(`/{user}/search?q=`)は**要ログイン**(302でログインへ)→ fromUser 非対応

## はてなブックマークのログイン差分確認(2026-07-09。ユーザーがログイン後にGUI操作+WebFetch)

未ログイン状態(2026-07-07 GUI操作採取)と、ユーザーがアカウントを作成・ログインした状態を突き合わせ。比較にはWebFetch(Cookie無しの独立した取得経路)を併用し、確証のある差分判定にした。

- **セーフサーチ(新発見)**: 検索結果サイドバーに「セーフサーチ」オン/オフのリンクがあり、`?safe=off`でオフにできることをログイン済みGUI操作で発見。**WebFetch(未ログイン相当)で同じURLを叩いても同じサイドバー・同じ既定(on)が出ることを確認**=ログイン差分ではなく、2026-07-07の未ログイン調査で単純に見落としていた既存機能。`safeSearchOff`として新規実装(既定=on、offのときだけ`safe=off`を送る)
- **ユーザー指定検索(fromUser)の再検証**: `/{username}/search?q=`というパスの存在は把握済みだったが、「ログインすれば使えるのか」を実際に確かめた。
  - 未ログイン(WebFetch)で`/{他人のuser}/search?q=`を叩く→ `www.hatena.ne.jp/login`へ302リダイレクト
  - **ログイン済みブラウザで`/{他人のuser}/search?q=`を開く→ 403 Forbidden**(対象ユーザーのブックマーク一覧ページ自体は公開・閲覧できることを確認済み=ブックマークが非公開なのではない)
  - ログイン済みブラウザで**自分自身の**`/{自分のid}/search?q=`は正常に動作(タイトルが「マイブックマーク検索」になる)。他ユーザーのプロフィールページに表示される検索ボックスも、実際に送信すると`/{そのユーザー}/search`ではなく通常のサイト全体検索`/q/...`に飛ぶことを実操作で確認
  - **結論**: この検索機能は「ログインすれば任意のユーザーのブックマークを検索できる」ものではなく、「ログインした本人が自分自身のブックマークだけを検索する」個人用機能。DialectのfromUser(指定した任意ユーザーの投稿を探す)とは性質が異なるため、**引き続き非対応(none)が正しい**。注記文言は「ログインが必要」ではなく実態(自分専用)に訂正
- 検証手段: ログイン済みブラウザでのGUI操作(サイドバー実クリック・プロフィールページの検索ボックス実送信)＋WebFetch(Cookie無し)でのクロスチェック。lint/build/check:operators/check:urls通過

## Mastodon(mstdn.jp、2026-07-03調査・以後 mastodon.social に切替済み)

- mstdn.jp は **Mastodon v4.3.22**(API実測)。検索ページの `?q=` をWeb UIが読む機能は **v4.4.0 で初実装**(PR mastodon/mastodon#32949)のため、**検索語入りURLが組めない**(開いても検索欄は空)
- 全文検索は要ログインのうえ、対象は検索許可をオプトインしたユーザーの公開投稿のみ(4.2以降の仕様)
- 未ログインで機能する公開URLは**タグページ `/tags/{タグ}` のみ**(裏の公開API・RSSも実測で確認)
- **この版差問題を避けるため、既定インスタンスを常に最新版のフラッグシップ `mastodon.social` に変更して実装**(下記2026-07-08調査を参照)

## Mastodon(mastodon.social、2026-07-08実装。GUI操作・ログイン済み)

- **検索ページの自動実行を確認**: `mastodon.social/search?q=...&type=statuses` はSPAだが、Misskeyと違いURL遷移だけで検索が自動実行される(手動ボタン不要)。ハッシュタグ単独は `/tags/{tag}` でログアウトでも閲覧可能
- **本文検索は実質ログイン必須**: 未ログインだと"hello"のような一般語でも0件になる(検索オプションパネルも「ログイン時のみ利用できます」と表示)。ログイン後は本文検索が機能する(このインスタンスが把握している投稿=自分の投稿・フォロー先・連合内で見つかった投稿に限られる)
- **演算子はログイン済みブラウザで実際の投稿の増減を見て裏取り**(いずれも✅):
  - `-語`(除外): 除外語を足すとその語を含む投稿が結果から消える
  - `"..."`(完全一致): 語順を入れ替えると0件になる=真の完全一致(tumblrと同じ確認方法)
  - `from:`(送信者、リモートは`from:user@host`): 投稿者で正しく絞り込まれる
  - `before:`/`after:`(期間): 日付の前後で異なる投稿に絞られる
  - `-is:reply`(返信除外): リプライ投稿が結果から消える
  - `language:`(言語): 指定言語の投稿だけに絞られる
  - `has:media`/`has:link`: 独立した絞り込みで、それぞれ異なる投稿に変わる(メディア添付 vs リンクカード)
- 出典: 公式操作ガイド(doc、Luca Hammerの演算子ガイド)＋2026-07-08のGUI操作(ログイン済み)による実測。検索オプションパネル自体はログイン時のみ表示されるため、パネルの網羅性はこのGUI操作で確認済み(has:media/poll/embed・is:reply/sensitive・language・from・before/during/after・in:all/library/publicの一覧を実見)

## Pinterest(2026-07-08実装。GUI操作・未ログイン)

- `pinterest.com/search/pins/?q=` は未ログインでも検索可能(要ログインではない)
- **フィルターパネル(調整アイコン)は「すべてのピン/動画/ボード/プロフィール」の4択のみ**。期間・並び順・送信者・ハッシュタグの絞り込みは無い。選ぶとパスが `/search/{pins|videos|boards|users}/` に切り替わることを実機確認(適用ボタン押下後のURL遷移で確認)
- **公式のBoolean演算子は無い**: `-語`(除外)を試したところ、Pinterest自身が「"black cat orange"の検索結果です。"black cat -orange"で検索し直しますか?」という案内を表示し、`-`をそのまま検索語の一部として扱う(除外にならない)ことを実機確認。引用符による完全一致も、"black cat"(quoted)で無関係な結果(Air Jordanのスニーカー)が混ざるなど、信頼できる文字列一致ではなくML/おすすめベースの緩い一致であることを確認
- 上部の「すべてのピン/自分のピン」切替はログイン後の個人スコープ(自分の投稿)で、Dialectが対象とする一般公開検索とは別軸のため対象外(Mastodonの `in:library` と同種の判断)
- 実装方針: keywords/exactPhraseは共にpartial(ゆるい一致、note.loose.and/note.loose.exact)、excludeは非対応(none、除外記号がそのまま検索語に混入する実害があるため`note.pinterest.exclude`で明記)、resultTypeはfull(pins=既定/videos/boards/users)。boardはPinterest専用の新しい値としてResultType型に追加(video/peopleは既存値を再利用)

## FANBOX(2026-07-08実装。GUI操作・未ログイン)

- 検索欄は「クリエイター・タグを検索」の1つのみ(トップページ・ヘッダー共通)。**投稿本文の全文キーワード検索は存在しない**
- `fanbox.cc/search?type=creator&q=` はクリエイター名の部分一致検索。`type=tag` はタグ名候補のサジェスト一覧(件数付き)で、クリックすると `/tags/{タグ}` のタグページへ遷移する
- **複数語のタグ検索はAND投稿検索にならない**: `?type=tag&q=猫 犬` を試したところ、「猫」と「犬」の両方の文字を含むタグ名(「犬猫」「犬や猫と幸せに暮らす」等)の候補一覧が出るだけで、投稿の絞り込みではないことを実機確認
- 投稿一覧として機能する唯一の経路は単一タグのページ `/tags/{タグ}`。**並び順・除外・期間・送信者などの詳細フィルタは一切無い**(ページ内の操作可能要素は検索ボックスのみ、`read_page`で確認)
- **ログイン状態**: FANBOXはpixivアカウントと別セッション(pixiv本体がログイン済みでもFANBOXは未ログインのまま)。未ログインでもタグページの投稿一覧(タイトル・作者名・件数)は閲覧できる。個別投稿の本文は支援額に応じてロックされるが、これは検索機能ではなくコンテンツ課金の制約
- 実装方針: ユーザー判断により「タグのみで実装する」を選択。`hashtag`のみ対応(partial、単一タグのときだけ`/tags/{タグ}`。複数タグや他の条件と組み合わせた場合は検索不可としてnull)。keywords等の全文検索概念は非対応(未記載=none)

### FANBOX: イラスト観点再監査(2026-07-13、#70。GUI操作・ログイン済み)

- **ログイン要否の切り分け完了**: ユーザーがpixivアカウントでpixiv本体→FANBOX個別の順にログイン(FANBOXはpixivと別セッションのため、pixivログイン後もFANBOX側で改めてログイン操作が必要と実機で再確認)。ログイン済み状態でも `/tags/{タグ}` の表示要素・件数・並び順は未ログイン時と完全に同一で、検索/フィルタ面での差は無いと確認(比較して差の有無を確認した、確証あり)
- **アカウント設定に「R-18コンテンツを表示する」トグルを発見**(`fanbox.cc/user/settings`の「閲覧設定」、要ログイン)。ただしこれはURLパラメータではなくアカウントに紐づく永続的な表示設定(pixiv本体のR-18設定と同種)で、Dialectの「URLに演算子を埋め込んだディープリンク」というアーキテクチャの対象外(ログインセッション依存でリンク単体では再現できない)。年齢区分の横断概念への対応先としては使えないため見送り(実装コストの見送りではなく構造的に対象外)
- 結論: イラスト観点の他の観点(AI生成除外・人気絞り込み・一次/二次創作タグ・画像形状/解像度・制作ツール)も含め、FANBOXにはこれらを載せる先のフィルタUI自体が存在しない。新規フィールドは無し(既存のhashtag実装のまま)

## bilibili(2026-07-08実装。GUI操作+URL叩き・未ログイン)

- `search.bilibili.com/{タブ}?keyword=` は未ログインで検索可能(日本からのアクセスで地域ブロックなし)。ログインを促すパネルは出るが検索・結果表示とも機能する
- **タブ(探すものの種類)**: 综合`/all`・视频`/video`・番剧`/bangumi`・影视`/pgc`・直播`/live`・专栏`/article`・用户`/upuser`。タブの実クリックでパスが切り替わることを確認
- **並び順(`order=`)はタブごとに違う**(すべてボタンの実クリックで採取):
  - all/video: `click`(最多播放)/`pubdate`(最新发布)/`dm`(最多弹幕)/`stow`(最多收藏)。既定は综合排序(パラメータなし)
  - article: `pubdate`(最新发布)/`click`(最多点击)/`attention`(最多喜欢)/`scores`(最多评论)
  - live: `live_time`(最新开播)のみ
  - bangumi/pgc: 並び順なし。upuser: 粉丝数・Lv等级順のみ=Dialectの並び順概念に対応しないため見送り
- **「更多筛选」パネル(all/videoのみ)**: 日付範囲(全部日期/最近一天/最近一周/最近半年＋開始日期・結束日期の任意指定)と動画の長さ(全部时长/10分钟以下/10-30分钟/30-60分钟/60分钟以上)。「分区筛选(ジャンル)」は「维护升级中(メンテ中)」で使えない(2026-07-08時点)
  - 日付範囲→ `pubtime_begin_s`/`pubtime_end_s`(**中国標準時UTC+8のエポック秒**)。「最近一天」の実クリックで採取し、2026年6月の任意範囲をURL叩きで指定して新着順の先頭が06-30になることを実測
  - 動画の長さ→ `duration=1`(10分未満)/`2`(10-30分)/`3`(30-60分)/`4`(60分超)。ボタンの実クリックで1〜4すべて採取
- **除外(`-語`)は効かない**: `猫 -狗` で犬の動画が結果に残り、`-狗`が文字として検索語扱いされることを実測(タイトルにも「猫 -狗」と表示される)。excludeはnone+注記(note.exclude.literal)で実装
- **引用符の完全一致も効かない**: `"猫狗到底"` で語順・連続性を無視した緩い一致の結果が返る(該当フレーズを含まない投稿が多数)。exactPhraseはpartial(note.loose.exact)
- 実装方針: resultTypeに新しい値 bangumi/pgc/live/article を追加(video/channelは既存値を再利用)、SortOrderに新しい値 danmaku(弾幕数順)/favorites(収蔵数順)/likes(いいね順)を追加。タブと並び順・長さ・期間の組み合わせ制約はdynamicSupportで表現(使えない組み合わせは「使えない」に落として注記)

### bilibili: イラスト観点再監査(2026-07-13、#70。GUI操作・未ログイン)

- 「更多筛选」パネルをJS実機操作で再度開き、内容が2026-07-08と完全一致することを確認(日付範囲・動画の長さのみ)。「分区筛选(ジャンル)」は今回も「维护升级中，敬请期待」(メンテ中)のままで変化なし
- AI生成除外・年齢区分に相当するフィルタはこのパネルに存在しない。bilibiliには青少年模式(未成年者向けモード)があるが、これはブラウザ/アカウント単位の永続設定でURLパラメータではないため、他サイトのR-18トグル・Restricted Modeと同じ理由で対象外と判断(今回は未ログインのため実機で該当UIには到達していないが、モード自体の性質から構造的に対象外と判断できる)
- **ログイン要否**: 2026-07-08時点で「ログインを促すパネルは出るが検索・結果表示とも機能する」と確認済み。今回の再監査でも未ログインで「更多筛选」パネルが問題なく開閉できることを確認(ログイン専用UIへの誘導なし)。bilibiliは海外からの本登録に電話番号認証等の障壁があり、ユーザーは未保有・今回のための新規作成もしないため、CLAUDE.mdの「確信が持てるか」判定に従う: フィルタパネルの構造(未ログインでも全項目に到達でき、ログイン限定の追加UIが見当たらない)から「未ログインで完結する」と確信を持って判定
- 専栏(コラム/article)タブは並び順(pubdate/click/attention/scores)のみで、動画系タブと違い日付・長さの絞り込み自体が無い(2026-07-08時点の記録通り、今回も変化なし)
- 結論: bilibiliに新規フィールドは無し(既存実装のまま)

## Fantia(2026-07-09実装。GUI操作・ログイン済み)

- 検索(`fantia.jp/posts?keyword=`)は**要ログイン**(未ログインで開くと「検索機能…はログインが必要です」バナーつきでログインページへリダイレクト、2026-07-08確認)。ユーザーがアカウント作成・ログイン後、ログイン済みブラウザでヘッダーの「さがす」→キーワード入力→「投稿」タブを実操作して採取
- 検索結果は6タブ(クリエイター`/fanclubs`・無料チケット`/coupons`・投稿`/posts`・商品`/products`・コミッション`/commissions`・タグ`/tags`)に分かれる。**投稿(`/posts`)だけが他サイトと同種の「本文を含む全文検索」**で、他5タブはクリエイター一覧・商品販売・コミッション受付・人気タグ発見など性質が異なるため今回は対象外(未実装のまま残す。方針上「見送り」ではなく「未着手」)
- **キーワードはAND**(スペース区切り、`猫 メイド`の両方を含む投稿だけがヒットすることを実機確認)
- **除外(`-語`)は効かない**: `猫 -犬` が0件になり、`-犬`という文字列そのものが検索語として要求される(該当する投稿が無いため0件)ことを実測。exclude はnone+注記(note.exclude.literal)
- **完全一致の引用符も効かない**: `"猫耳メイド"`(引用符つき)が0件になる一方、同じ文字列を引用符なしで検索すると多数ヒットすることを実測。引用符ごと文字列として要求されるため一致しない。exactPhraseはnone+専用注記(note.fantia.exactLiteral)
- **キーワード検索範囲(`keyword_type=`)**: 詳細検索パネルのプルダウンで「タイトルのみ」(`title_only`、既定)/「タイトルと本文」(`all`)を切替(実測)。既存のtitleOnly概念と対応するが、**Fantiaは既定がタイトルのみ**なので、他サイトと逆に「タイトルのみを外す」ときだけ明示的に`keyword_type=all`を送る必要がある
- **カテゴリー(`category=`)**: 詳細検索パネルの「カテゴリー」チップ(イラスト・漫画・その他グループ19種＋実写グループ5種、計24種)を実操作しJSでinput要素のname/valueを採取(`illust`/`comic`/`vtuber`/`voice`/`voiceactor`/`3d`/`2d_anime`/`game`/`music`/`novel`/`doll`/`art`/`program`/`handmade`/`history`/`railroad`/`shop`/`other`/`fortune`/`cosplay`/`idol`/`youtuber`/`photo_movie`/`other_real`)。**サイト側は複数選択可**(`category=illust,cosplay`のようにカンマ区切り、両グループ跨ぎでも同時選択できることをUI操作で確認)だが、Dialectは他の種類系概念(ジャンル等)と同じ単一選択で実装
- **対象読者(`brand_type=`)**: 全年齢`3`/男性向けR18`0`/女性向けR18`2`の3タブ。**このパラメータを省略すると、その時点のアカウント/セッションの直近の閲覧設定が使われる**ことを実測(何も操作していないのに`brand_type`省略時のタイトルが「(男性向け)」になっていた)。全年齢に固定されないため、Dialectは常に明示的に送る(既定=`3`)
- **並び順(`order=`)は5値**(セレクトの値をJS/キー操作で全て確認): `newer`(投稿の新しい順、既定)/`create_old`(投稿の古い順)/`updater`(更新の新しい順)/`update_old`(更新の古い順)/`popular`(お気に入り数順)。Dialectは`newer`→new、`popular`→topの2値のみ対応し、残り3値(更新順・古い順)は未着手のまま残す(採否選定はせず、実装コストの分割として後回し)
- 期間・送信者(投稿者)によるフィルタは詳細検索パネルに存在しない(カテゴリーとキーワード検索範囲のみ)
- 実装方針: group=image(pixiv/ニコニコ静画/FANBOXと同じ家族)。brandColorは検索結果ページの主要ボタン背景色から採取(`#EA4C89`)。simple-iconsに公式ロゴが無いため、バブル代替(「フ」)で表示

### Fantia追記(2026-07-13、issue #70)

- **GUI操作(ログイン済み)**。`fantia.jp/posts?keyword=...`の並び順プルダウン横「絞り込み」ボタンから開く「詳細検索」モーダルで新発見: **R18指定(`rating=`)** のプルダウンがあり、`all`(すべて、既定・未指定相当)/`general`(全年齢)/`adult`(R18)の3値。**対象読者(`brand_type=`、男性向け/女性向けタブ)とは独立した別軸のフィルタ**で、両方を同時に送れることをモーダル送信で実測(`rating=general`を選んで送信→`category=illust&product_categories_name=&product_type=&rating=general&order=newer&keyword_type=title_only&keyword=...`が生成された)。既存のageRating概念(pixivと共有)をFantiaにも統合して実装
- 並び順の残り3値(2026-07-09時点で「未着手」と記録していた`order=updater`/`update_old`/`create_old`)も実装。SortOrder型に`updated`(更新の新しい順)/`updatedOld`(更新の古い順)/`oldest`(投稿の古い順)を追加し、Fantiaのsupport.sortOrderをfullへ格上げ

## Twitch

- `twitch.tv/search?term=`(実ブラウザ実測)。ログイン不要
- 演算子は**一切効かない**(引用符・`-`除外とも実測で無効。あいまい一致)
- `type=channels|categories|videos` で対象切替(実測)。「配信中のみ」のtypeは存在しない
- 言語絞り込み(日本語のみ)は**URL不可**(`?lang=ja`は無視。UIの言語フィルタはURLに反映されないクライアント内部状態)

## 5ちゃんねる

- **2026-03にドメインが5ch.net→5ch.ioへ移行**(レジストラ剥奪)。公式スレタイ検索は find.5ch.io(旧find.5ch.netは301転送)
- 公式(find.5ch.io、検索エンジンv3)は**スペース区切りがANDにならない**(関連度ベース。実測で片方の語のみの結果が上位に混在)。除外・引用符も無効 → 「足す=絞る」を満たさず不採用
- **ff5ch.syoboi.jp(スレタイ検索)を採用**: `?q=` でAND(実測)・`-語`除外(実測)・`@板ID`板指定(実測)が全て有効。部分文字列マッチ型・スレ作成の新しい順固定・ログイン不要・5ch.io移行済みインデックス
- dig.5ch.net は死亡(522)。板指定は既存概念「このコミュニティの中だけ」(subreddit)に相乗り(先頭の1つだけ)

## あにまん掲示板

- 検索はパス埋め込み: `/search/{query}`(現行スレ・直近約2か月)・`/search2/{query}`(全期間の過去ログ)・`/searchRes/{query}`(レス本文・β)。いずれもスレタイトル検索(searchResのみ本文)
- スペース区切りANDは実測で有効。除外・引用符・並び順・期間のパラメータは**なし**(検索フォームの実パラメータはflagとqのみ)
- **部分文字列マッチ型**: 語中の断片「ンピュー」(コンピューターの中間)がレス検索(searchRes)で4件ヒットすることを確認(URL叩き 2026-07-10)。完全一致の語句はつながったまま効く(スペースを含む語句は語ごとに分かれる)
- Dialectの翻訳: 既定=レス本文(`searchRes`、βのため取りこぼし注記)、「タイトルだけで探す」ON=全期間過去ログ(`search2`)
- Cloudflare配下でボット的UAは403(ブラウザで開く分には問題ない)

## ニコニコ静画

- サービスは**稼働中**(2024年攻撃後も継続。終了したのは成人向けコーナーのみ)
- イラスト: `/search/{query}?target=illust_all`(キーワード)・`/tag/{タグ}`(タグ一致)。ログイン不要
- 演算子は件数比較で実測: スペースAND・`-語`除外(件数が完全に整合)・OR。引用符は無害だが効果未確定
- 並び順 `sort=`(検索・タグ共通): `image_created`(新着)/`image_view`(閲覧数=人気の代用)。サイト既定は`comment_created`(コメント新着順)のため、新着順は必ず明示する
- マンガは別ドメイン `manga.nicovideo.jp/search?q=`(実測で稼働)。並び順パラメータ未確認のため送らない → 概念「作品の種類」で切替

## 新概念「作品の種類」「探すもの」(2026-07-03)

URLで指定できる条件はすべて概念化する方針(対応1サイトでも追加してよい。ビルダーは対応サイト数順ソート+サイト絞り込みで吸収できる)。

- **作品の種類(workType)**: イラスト/マンガ。pixiv(パス `/illustrations`・`/manga`、未指定は`/artworks`)とニコニコ静画(静画検索⇔ニコニコ漫画検索)が対応
- **探すもの(resultType)**: 動画/チャンネル。YouTube(sp種別 type=1/2)とTwitch(`type=videos|channels`)が対応
- YouTubeのsp値はソート×長さ×種別の組み合わせが必要になったため、固定テーブルをやめ**protobufバイト列からの組み立て**に変更(実測済みの CAM%3D・EgIYAQ%3D%3D・CAMSAhgB・EgIQAQ%3D%3D の再現をnode検算で確認)

## 参考ソース(抜粋)

- はてブ: 実測フェッチ多数(2026-07-03)、[公式ヘルプ 検索機能](https://b.hatena.ne.jp/help/entry/search)
- Mastodon: mstdn.jp API実測(version/検索/タグタイムライン)、[公式ドキュメント user/network](https://docs.joinmastodon.org/user/network/)、[PR #32949(?q=対応、4.4.0)](https://github.com/mastodon/mastodon/pull/32949)
- Twitch: 実ブラウザ実測(2026-07-03)、[公式ヘルプ How does search work](https://help.twitch.tv/s/article/how-does-search-work)
- 5ch: find.5ch.io・ff5ch.syoboi.jp実測(2026-07-03)、[GIGAZINE(ドメイン剥奪)](https://gigazine.net/gsc_news/en/20260306-5ch-domain/)、JDim Issue #60(dig停止)
- あにまん: bbs.animanch.com実測(フォーム定義・3種の検索。2026-07-03)
- 静画: seiga.nicovideo.jp実測(件数比較27パターン相当。2026-07-03)、ニコニコインフォ(春画終了)

# Googleフォールバック(site:検索、2026-07-03実装)

本体サイトの検索で外れる条件を、Googleの `site:` 検索(サイト内検索)で補う機能。note公式ヘルプ自身が除外検索の代替として `site:note.com kw -除外` のGoogle検索を案内していることが着想の根拠(上記noteの節を参照)。

## 使う演算子(すべてGoogleの公式・文書化済み構文のみ)

| 概念 | Google構文 | 備考 |
| --- | --- | --- |
| キーワード | スペース並置(AND) | |
| どれかを含む | `(a OR b)` | 括弧+大文字OR |
| 完全一致 | `"..."` | |
| 除外 | `-語` | |
| 期間 | `after:YYYY-MM-DD` / `before:YYYY-MM-DD` | 2019年に公式化(Danny Sullivan発表) |
| 日本語のみ | URLパラメータ `lr=lang_ja` | |

# 定期確認: 2026-07-04 全項目通し(ブラウザ+curl併用)

[operator-checklist.md](operator-checklist.md) の全項目を並列で実測。手段は使い分け(はてブ・ff5ch・find.5ch.io・あにまんは `curl`+grep、SPA/要ログインのX・Misskey・Instagram・Bluesky・Twitch・YouTube・pixiv・note・niconicoはブラウザ)。結果は概ね従来どおりだが、以下3点を新たに確認・修正した。

## 1. Misskey.io の検索URLが自動実行されない(✅想定→❌)

- `misskey.io/search?q=台風&type=note` を開いても、クエリ欄に「台風」が入り「検索」ボタンが表示されるだけで**結果が自動表示されない**(6秒待機後も検索フォームのまま)。`username=` 付きでもユーザー指定欄は正しく埋まるが同様に結果は出ない。
- 従来のチェックリスト✅はWeb調査ベース(ルーター定義)で、URL遷移=結果表示になる前提だった。初のブラウザ実測でこの前提が崩れた。
- 影響: Dialectが生成するMisskey検索ディープリンクは、ユーザーが遷移先で手動クリックしないと結果が出ない=実質機能不全。タグ単独ページ(`/tags/`)は自動表示されるので影響なし。**扱いは要検討(コード未変更)**。

## 2. pixiv `order=date_d` + `scd/ecd` がエラーページ(バグ修正)

- チェックリストの検証URL `pixiv.net/tags/猫/artworks?scd=...&ecd=...&order=date_d` は「エラーが発生しました」を返す。直後に `order=date_d` を外した同一URLは正常(1,243件・期間バッジ表示)= date_d が原因と確定(レート制限ではない)。
- `date_d`(新着)はpixivの既定と同じで冗長。[pixiv.ts](../src/core/platforms/pixiv.ts) の `if (state.sort === 'new') params.set('order', 'date_d')` を削除し、新着時は order を送らないよう修正。`popular_d`(人気・プレミアム限定)のみ明示する。

## 3. YouTube 視聴回数順(CAM系)は近似ソート

- `sp=CAM%3D`・`sp=CAMSAhgB`・`sp=CAMSBBABGAE%3D` はいずれも「4分未満」「動画のみ」等の長さ・種別フィルタは厳密に効くが、**視聴回数の並びは近似**で、低再生の動画が高再生より上位に混じることがある(厳密な全順序ではない)。チェックリストの期待文言から「厳密な降順」を外し「おおむね降順(近似)」に修正。UI上の実害は小さいため送信は継続。

## その他(✅で確定した項目)

- X: `min_retweets:`・`filter:blue_verified` を実ログインで初実測し✅化(いずれも全結果が条件を満たす)。`since:`/`until:` は境界がUTC基準で、JST表示だとuntil当日まで見えるため誤判定に注意。
- YouTube: `-語`(リテラル除外は有効・関連語は残る)・`intitle:`・`sp=CAMSBBABGAE%3D`(合成値)を実測し✅化。`sp=CAI%3D`(アップロード日順)は依然として関連度に古い投稿が混在し**復活せず**(送信は継続)。
- note ハッシュタグは `/hashtag/{tag}` がコンテスト系タグだと `/contest/` に転送されるが、タグ記事一覧は表示されるため✅。
- curlはnote/niconicoの検索結果を取得できない(クライアント描画のSPAでガワのみ)。あにまん `/search2/` はCloudflareでcurl 503→ブラウザで確認。これらはブラウザ必須。

上記以外の概念(ユーザー指定・ハッシュタグ・メディア等)はGoogleへ翻訳できないため引き継がない。UI上は「◯◯はGoogleには引き継げません」と注記する。

## 提示ルール(src/core/google.ts)

- トリガー: ネイティブ検索で**外れた(dropped)**条件のうち、どれかを含む/完全一致/除外/期間 のいずれかがあるときだけGoogleボタンを出す(近似=partialは本体側で一応動くため対象外。ノイズ回避)
- Googleへ渡せる正の条件(キーワード・OR・完全一致)がなければ出さない(site:単独ではサイト全ページが対象になるため)
- 各サイトのドメインは PlatformDef.googleSite(x.com / bsky.app / youtube.com / note.com / nicovideo.jp / threads.com / instagram.com / tiktok.com / facebook.com / reddit.com / pixiv.net / misskey.io)

## 実機確認(2026-07-03、ブラウザで検証)

- `site:note.com 台風 -広告` → note.comの記事のみが並び、除外が有効
- `site:note.com 台風 -広告 after:2026-06-01` → 期間指定つきで動作
- `site:note.com (台風 OR 大雨)` → OR指定で動作(ネイティブ側が起動不能なOR専用ケースの救済)

## 限界(注記済み・許容)

- Googleのインデックス反映には時間差があり、直近の投稿はヒットしない(リアルタイム性はネイティブ検索が上)
- ログインウォールのあるサイト(Instagram・Facebook等)はインデックス範囲が部分的

# フィールド網羅・文法監査(ultracode、2026-07-04)

13サイト＋共通コアを多エージェント(静的整合性＋サイト別調査＋敵対的検証、24件採用/11件棄却)で監査した結果と、それを受けた実装変更。結論=網羅はほぼ完了・URL文法は健全だが、「UIが適用と表示するのに実際は落ちる/開けない」食い違いが数件あったため修正した。

## 1. 単独フィールド検索がガードで握りつぶされていた(最重要)

`hasPositiveTerm`(terms/exactPhrase/fromUser/hashtag のみを正の条件とみなす)を X/Bluesky が、独自ガードを Reddit/5ch が使っており、それ以外のフィールドだけを入れた検索が `buildUrl`→null になっていた。一方 `resolve` は静的 support だけを見て applied に振り分けるため、LaunchPanel は「1/1 条件を適用」と緑表示しつつボタンは無効(url=null)という食い違いになる(UI実コードで裏取り)。

- 修正: X は `to:`(toUser)・`url:`(domain)単独、Bluesky は `mentions:`(mentionsUser)・`domain:` 単独、Reddit は `subreddit:` 単独を正の条件に追加。いずれも各サイトで実在する単独検索。

## 2. resolve が実態を反映しない(パス分岐での取りこぼし)

`resolve` が静的 support レベルだけで applied/approximated/dropped を決めていたため、`buildUrl` が状態次第で概念を落とすケースを「適用」と誤表示していた。→ `PlatformDef.dynamicSupport?(state)` を追加し、状態に応じて support を上書きできるようにした(resolve が静的 support にマージ)。適用先:

- **YouTube**: ユーザー指定時はチャンネル内検索URLに切替わり sp= を送れない → 並び順・動画の長さ・探すものを `none`(注記 `note.youtube.channelConflict`)へ落とす。
- **Instagram**: ハッシュタグ2つ以上はタグページでなくキーワードSERPに落ちAND保証がない → hashtag を `partial`(`note.instagram.multiTag`)へ。
- **はてブ**: ハッシュタグ単独のときタグ検索パスになり titleOnly が無視される → titleOnly を `none`(`note.hatebu.titleTagConflict`)へ。

## 3. 網羅の穴(実在演算子の追加)

- **X `url:`**(domain概念): リンク先ドメインで絞る演算子。support.domain=full＋`url:<domain>` を追加(igorbrigadir参照)。→ **2026-07-04ログイン済みブラウザで実測✅**(`台風 url:nhk.or.jp` の結果本文に www2.nhk.or.jp リンク)。
- **5ch 複数板 OR**: `@板ID` を先頭のみ→ words(subreddit) 全部送信に。→ **2026-07-04 curl実測✅**(`テスト @sony @news4plus` の結果に sony板・news4plus板の両スレIDが混在)。
- **pixiv タイトルだけ**(titleOnly、s_mode=s_tc): タイトル・キャプション検索。公式ヘルプ記載。→ **2026-07-04ブラウザ実測✅**(「タイトル・キャプション」検索として結果表示。pixivは内部で s_mode=tc に正規化)。support.titleOnly=partial(タグではなく本文相当を対象)で追加。
- **Misskey 除外・フレーズ**: 本文検索はMeilisearchなので `-語` の除外(support.exclude none→partial)と `"..."` のフレーズ一致(exactPhrase を引用符で括る)を送るようにした。→ **2026-07-04ログイン済みブラウザで暫定確認⚠️**(検索実行後、猫を含み犬を含まない結果。厳密な対比は次回)。キーワード注記も「本文の部分一致」→「語ごとのAND」に修正。リモートユーザー `@user@host` は `username=`＋`host=` に分離。
- **Reddit `self:no`(linksOnly): 撤回**。当初はテキスト投稿を除く=リンク投稿だけとして追加したが、**2026-07-04ブラウザ実測で現行の reddit.com/search/ が `self:` を無視することが判明**(`self:yes` でも画像投稿が並び `self:no` と結果同一)。「適用と出るのに効かない」不具合になるため送信を撤回し、linksOnly対応を外した。テキスト投稿だけに絞れるようになったら復活(checklistに復活チェック行)。

## 4. 表記・軽微

- **YouTube 動画長**: 2026-01の再編でUIラベルが「4分未満/4〜20分/20分超」→「3分未満/3〜20分/20分超」に変わった。送信するsp protobufバイト(0x01/0x03/0x02)は不変で正しくバケットに効くため送信は変更なし。DialectのUIラベルは niconico(4分基準の枠組み)と共有のため据え置き。
- **はてブ minLikes**: 任意の `users=N` が最低ブックマーク数として効く(実測 users=7/25)。「選択肢にない数は効かない可能性」の注記は誤りだったので、ブックマーク数として絞る旨に修正(partialは概念の使い回し明示のため維持)。

## 実機裏取り(2026-07-04 実施済み)

追加した演算子を実機で確認した(curl＋ログイン済みブラウザ)。結果は上記各項目に反映済み: X `url:`✅ / 5ch 複数`@板`✅ / pixiv `s_mode=s_tc`✅ / Misskey `-語`・`"..."`⚠️(暫定) / Reddit `self:no`❌(撤回)。Misskeyの厳密な対比(`-犬`が犬投稿を実際に除くか)は次回に持ち越し→**2026-07-08に再検証・確定**(除外✅／完全一致は機能しないと判明。詳細は上記「Misskey.io」節の「除外・完全一致の再検証」参照)。

## 共通概念の拡張(2026-07-04 実装)

当初「1サイト専用の選択肢を共通enumに足すと散らかる」として見送りかけたが、これはプロジェクト方針(「URLで指定できる条件はすべて概念化する。対応1サイトでも追加してよい。対応サイト数順ソート＋サイト絞り込みで吸収」)および既存の1サイト専用フィールド(excludeUser/toUser/mediaOnly/linksOnly/verifiedOnly/minReposts=X専用、mentionsUser=Bluesky専用、workType=pixiv専用)と矛盾するため撤回。値レベルの他サイト非対応は dynamicSupport(共通ヘルパー `limitSort`)で「適用と出るのに送られない」を防げる。以下を実装:

- **resultType に short(ショート)/playlist(再生リスト)を追加**: YouTubeのタイプ。sp種別バイトは video=1/channel=2/playlist=3(既存調査)、**short=9 は2026-07-04にYouTubeフィルタUIから実測**(単体sp=EgIQCQ%3D%3D)。Twitchはショート/再生リストを持たないため twitch.ts の dynamicSupport で resultType を落とす。
- **sortOrder に hot(急上昇)を追加**: note の `sort=hot`。**2026-07-04ブラウザ実測で動作確認**(タイトル「急上昇の記事一覧」・急上昇タブ選択)。note以外の並び順対応サイト(X/Bluesky/YouTube/niconico/Reddit/pixiv/はてブ)は `limitSort(sort, ['new','top'], …)` で hot 選択時に sortOrder を非対応へ落とす。
- **pixiv タイトルだけ(s_mode=s_tc)**: 既存 titleOnly 概念の再利用(上記「網羅の穴」参照)。

### 見送り(実装しないと判断)

- **note `sort=like`(定番・スキの多い順)**: `sort=like` は実測で動作(タイトル「定番の記事一覧」・いいね5万→1.4万の降順)するが、既存の「人気順(top→popular)」と用途が重複しがちで、並び順ボタンが5個になり幅も溢れる。UXコストが価値を上回るため入れない。
- **YouTube movie(映画)**: sp種別バイト未確定(採取が不安定)＋用途がニッチ(有料映画)。
- **Twitch categories(type=categories)**: ゲーム/ディレクトリ閲覧でありキーワード検索の対象として薄い。
- 真にサイト固有性が強い絞り込みを大量に増やすなら、共通enumではなく「サイト固有オプション」の仕組みを別途設計するのが筋。

## 既存サイトの採取監査(2026-07-07、GUI操作)

2026-07-07にユーザーから「既存サイトもTumblrの教訓通り(詳細検索/フィルタGUIまで開き尽くしてから済みとする)に再点検してあるか」と問われ、大半が未着手と判明(→ dialect-backlogメモリ「既存サイトの採取監査＋出所記録の欠落」)。その対応として X→Reddit→pixiv→YouTube→はてなブックマークの順で、実際にブラウザで検索フォーム・詳細検索/フィルタパネルを操作し、生成URLを採取した。**すべて出所タグ=GUI操作、2026-07-07**。ログイン状態はX/Reddit/pixiv/YouTubeともログイン済みセッション、はてなブックマークは未ログインで確認。

### X(`x.com/search-advanced`、GUI操作)

フォーム全項目を実操作。既存実装との差分:
- **新発見: 「次のアカウントへの@ツイート」(mentions)欄が存在** — 1件入力で `q=(@user)`、2件で `q=(@user1 OR @user2)` を生成(実機確認)。現行 `toUser`(`to:`宛先)とは別物で、**Xのメンション検索は未対応**という既知の穴([[field-coverage-audit]])を実機構文つきで確定させた。dialectには対応する概念がない。
- 「フィルター」欄は**返信・リンクの2つのみ**(画像/動画/認証済みのトグルは無い)。現行コードの `mediaOnly`(`filter:media`)は full 表示で undocumented 注記が無いが、公式フォームには2026-07-07時点で存在しない(`verifiedOnly` と同様の「フォームから消えた非公式演算子」)。動作可否自体は今回未検証(注記の要否は別途判断)。
- 「返信」トグルには「返信のみ表示」ラジオもあり(=`filter:replies`単独、現行の除外`-filter:replies`と逆方向)。未対応・低優先。
- 「いずれかを含む」(OR)は方針([[additive-narrowing-principle]])により意図的に非対応。他の全欄(すべて/完全一致/含まない/ハッシュタグ/言語/from/to/エンゲージメント3種/日付since・until)は現行実装と一致。

### Reddit(`reddit.com/search`、GUI操作)

- **新発見: `type=` パラメータ未送信** — 結果タブは すべて/投稿/コミュニティ/コメント/メディア/プロフィール で、それぞれ `type=`(無指定)/`posts`/`communities`/`comments`/`media`/`people` を生成(実機確認)。現行 `buildUrl` は `type=` を一切送らないため、常に「すべて」(投稿+コミュニティ+コメント+メディア+プロフィールの混在)を返している。**`mediaOnly` は現行 `{level:'none'}` だが実際は `type=media` で実現可能** — 誤り。resultType 概念(posts/communities/comments/media/people)を足すか、既定を `type=posts` にするかは方針判断が必要。
- **新発見: 並び順に「コメント数」(`sort=comments`)が存在**(実機確認)。現行 sortOrder は new/top/hot の3値のみ。
- 並び順の他選択肢(関連度=既定/注目順=hot/賛成票数順=top/新しい順=new)・期間(すべての時間/過去1年/過去1か月/過去1週間/今日/過去1時間 → `t=`all/year/month/week/day/hour)は現行の `tParam` ロジックと一致。
- 「セーフサーチ」トグルはアカウント設定でURLパラメータ化されない(クリックしてもURL不変を確認)ため対象外。

### pixiv(`pixiv.net` 検索オプションパネル、GUI操作)

- **新発見: 「対象」に「タグ・タイトル・キャプション」(`s_mode=tag_tc`)が存在** — タグ完全一致/部分一致/タイトルキャプションに続く4番目の値(実機確認、URL `/search?q=...&s_mode=tag_tc&type=artwork`)。現行は s_tag/s_tag_full/s_tc の3値のみ実装。
- 「作品の種類」に「イラスト・うごくイラスト」(マンガを除く組み合わせ)があるが、現行のillust/manga/novel/ugoira/既定(全部)の組み合わせにはない中間値。低優先。
- 「作品の詳細」に解像度・縦横比・制作ツールの3絞り込みが存在(実機確認、未実装)。これは2026-07-06の [[field-family-map]] 設計で既に「耐荷重の主戦場=制作ツール~120」として認識済みの未着手事項で、新発見ではなく現存確認。
- 「ユーザー」タブは作品タグ検索と別機能(ユーザー名検索)であり、タグ検索に `author:` 相当の演算子は無いことを確認 — 現行 `fromUser: {level:'none'}` は正しい。
- ブックマーク数(プレミアム限定)・年齢制限・AI生成表示・投稿日時は既存実装と一致。

### pixiv「作品の詳細」調査(2026-07-13、GUI操作、issue #70) — 実装後に撤回

2026-07-07に上記で「実機確認、未実装」と記録していた「作品の詳細」の解像度・縦横比・制作ツールの3項目を、GUI操作(ログイン済み、issue #70)で確認した。pixiv「検索オプション」モーダル(ログイン限定機能。`/tags/{q}/artworks`ページの検索オプションボタンから開く)の「作品の詳細」で解像度・縦横比・制作ツールの3項目を実機操作し生成URLを採取。解像度=`wlt`/`wgt`/`hlt`/`hgt`(4パラメータの組み合わせでlarge/medium/smallの3バケット)、縦横比=`ratio`(0.5/-0.5/0)、制作ツール=`tool`(103種、生の値をそのまま送る)。いずれも`/tags/{q}/artworks`パスに直接付与しても有効(searchエンドポイントへの切替は不要、実機確認済み)。

**2026-07-13、ユーザー判断によりissue #70で実装後に撤回。ファンメイド/任意テーマの創作物を探す利用文脈に合致しない技術的付随情報と判定(CLAUDE.md「フィールドの採否選定はしない」の利用文脈適合性テスト参照)。** 存在自体は確認済みのため、将来の監査でこの3項目を再発見しても実装しない(docs/operator-checklist.mdでは🚫実装見送りとして記録)。

今回のGUI操作で隣接する2つの発見があったが、いずれも本ラウンドでは意図的に未実装のまま残す(採否選定ではなく設計待ち・調査継続の扱い):

- **`csw=1`(「作者でまとめる」)**: 検索オプションモーダルにあるトグルで、結果を投稿者単位でグルーピングする表示オプション。結果を絞り込む「概念(concept)」ではなく表示の見せ方の切り替えのため、既存のConceptId/QueryStateの枠組みにきれいに収まらない([[field-family-map]]でいう「表示オプション」的な新しい機構が要る)。設計待ちのフォローアップとして残し、採否却下ではないことを明記する。
- **`illust`ワークタイプの粒度**: `/tags/{q}/artworks`上、pixiv自体の「作品の種類」ドロップダウンは「イラスト・うごくイラスト」(結合)と「イラスト」(純粋イラストのみ)を別選択肢として提供しているが、現行の`illust` workType値は両者を区別できず、どちらも同じ`/illustrations`セクションパスに当たる(このセクション自体に区別用のパラメータが無い)。これは今回の変更で持ち込んだ問題ではなく、既存の`illust`値のラベリングが以前から粗かった可能性がある。`/tags/{q}/illustrations?type=illust`のようなURLが存在し`bare /illustrations`と違う挙動をするかどうかは未検証で、修正ではなく追加調査が必要な事項として残す。

### YouTube(`youtube.com` 検索フィルタパネル、GUI操作)

- 「タイプ」(動画/ショート/チャンネル/再生リスト/映画)・「時間」(3分未満/3〜20分/20分以上)は現行実装と一致(映画は既存方針通り見送り)。
- 「優先設定」(並び順)は**関連度順・人気度の2値のみ**(アップロード日順の選択肢が無い) — 2026-07-03に実機確認済みの「アップロード日順(CAI系)はソートが効かなくなっている」という既存の弱点([[dialect-backlog]])を、公式UIの選択肢自体が無いという形で再確認。
- **新発見: 「特徴」に10項目**(ライブ/4K/HD/字幕/クリエイティブ・コモンズ/360°/VR180/3D/HDR/場所/購入済み)のうち、実装済みは「ライブ」のみ。4Kのbyte値を実機解析(`sp=` 内 `filter` サブメッセージの field14=1)し、field8=ライブ(既知)とは別フィールド番号であることを確認、この形式は他Featuresにも同様に拡張可能と判断できる(字幕/クリエイティブ・コモンズ以外は個別解析未実施)。字幕・クリエイティブコモンズは実用価値がありそうだが今回は存在確認どまり。
- 「アップロード日」(今日/今週/今月/今年)も `sp=` のfilterサブメッセージ内(field8、今週=value3)で表現可能と実機確認。ただし現行の `after:/before:` クエリ演算子の方が任意日付を指定できて上位互換のため、置き換えの必要性は低い。

### YouTube: イラスト観点再監査(2026-07-13、#70。GUI操作)

- **ログイン要否**: 2026-07-08時点で「ログイン済みで見た選択肢が既存記録と一致=ログイン限定オプションなし」と既に判定済み(issue #70コメント)。今回はこの判定を踏襲し、ログイン比較はやり直さなかった
- 検索フィルタパネル(タイプ/時間/アップロード日/特徴/優先設定)をShadow DOM越しにJS実機操作で再取得し、上記2026-07-08の記録と完全一致することを確認(タイプ5種・時間3種・アップロード日4種・特徴11種・優先設定2種、すべて既存実装済み)。新規項目は無し
- イラスト観点の各項目を照合: AI生成の除外は無し。年齢区分に相当する「制限付きモード」はYouTubeのアカウント/ブラウザ単位の永続設定であり(かつYouTubeはそもそもポルノ的コンテンツを規約で禁止しておりpixiv/Fantia的なR-18二値とは性質が異なる)、URLパラメータではないためFANBOXのR-18トグルと同じ理由でDialectのディープリンク方式の対象外。人気絞り込みは`優先設定=人気度`(視聴回数順)として実装済み。一次/二次創作タグ・画像形状/解像度(HD/4K/HDRは実装済みだが縦横比は無し)・制作ツールブランドの概念はYouTubeの検索UIに存在しない
- 結論: YouTubeに新規フィールドは無し(既存実装のまま)

### はてなブックマーク(`b.hatena.ne.jp/search`、GUI操作)

- 検索対象(すべて/タグ/タイトル/本文)・並び順(新着/人気)・ブックマーク数プリセット・期間プリセット(すべて/1週間/1ヶ月/1年)+カスタム範囲入力、いずれも現行実装と一致。**新規の欠落は見つからず**。
- 「すべて」(`target=all`、タグ+タイトル+本文の統合検索)という値の存在は確認したが、現行の text 優先デフォルトと比べて曖昧さが増すだけで明確な価値がないため、追加候補としては優先度低。
- 期間無指定時の既定「直近5年・3users以上」は本監査でも再確認(既知の挙動、[[dialect-backlog]]記載通り)。
- ユーザー限定検索(特定ユーザーのブックマークに絞る)に相当するURL演算子は検索フォーム上に見当たらず、`fromUser: {level:'none'}` の現行判断は妥当と確認。

### まとめ

X/Reddit/pixiv/YouTubeの4サイトで実装漏れ・未実装の実在演算子を確認、はてなブックマークは漏れなし。採否は機能面の判断を要するため実装は保留し、[[dialect-backlog]] に一覧を追記してユーザー確認待ちとした。

## X `min_faves:`/`min_retweets:`/`min_replies:` の要フォーム再確認(2026-07-08、GUI操作)

2026-07-06に持ち越されていた宿題(「実在すればminLikes/minRepostsをfullへ格上げすべき」)を決着。`x.com/search-advanced` の「エンゲージメント」欄(返信の最小件数/いいねの最小件数/リポストの最小件数)に実際に値を入力して検索し、生成URLで3つとも確認: `min_faves:100`・`min_retweets:50`(min_repliesは2026-07-06に別途実測済み)。**旧記載「公式フォームからは削除済みの非公式演算子」は誤りと判明**(2026-07-07の既存サイト採取監査で「エンゲージメント3種は現行実装と一致」とすでに確認されていたが、support レベルとコード内コメントの追従が漏れていた)。対応: `src/core/platforms/x.ts` の minLikes/minReposts/minReplies を partial→full へ格上げ、誤ったコメントを修正。公式フォームから消滅済みなのは `filter:blue_verified`(認証済みトグル)のみ。

### X: イラスト観点再監査(2026-07-13、#70。推論・新規ログイン無し)

- X検索は全面ログイン必須(`requiresLogin: true`)。今回は使い捨てテストアカウントのBAN/凍結リスクをユーザーが懸念し、私自身が新規アカウントを作ることも規約上できないため、ユーザー判断で新規ログインを見送った(既存の実アカウントでのログインも選択肢として提示したが、既存調査ベースの推論確定を選択)
- 判定根拠: 2026-07-06〜07-11に何度もGUI操作で実測してきた`x.com/search-advanced`(「高度な検索」フォーム)は、キーワード・除外・ユーザー指定(from/to/mentions)・リンク先ドメイン・リスト・エンゲージメント数・言語・期間という投稿属性のみを扱うフォームで、pixiv/Fantiaのような作品分類概念(AI生成除外・年齢区分・一次/二次創作タグ・画像形状・制作ツール)は過去のどの監査でも一度も発見されていない
- 結論: Xに新規フィールドは無し(既存実装のまま)。ただし今回は「未ログインで完結する」の確証は取れておらず(構造的にログイン必須のため未ログイン比較自体が成立しない)、将来ユーザーが自身のアカウントでログインする機会があれば実機再確認が望ましい監査候補として残す

## 監査候補3件のGUI再点検(2026-07-09、GUI操作)

出所タグ台帳で「監査候補(表層調査の可能性が残る)」としていた Bluesky / note / niconico動画 の3件を、実際に検索フォーム・並び順・フィルタパネルをGUI操作して採取し直した。ブラウザは niconico=ログイン済み、note=未ログイン、Bluesky=ログイン済み。生成URLはブラウザのアドレス経由で採取。

### niconico動画(nicovideo.jp、GUI操作・ログイン済み)

**並び順ドロップダウン(全8値をGUI操作で採取)**。現行サイトが生成する並び順パラメータは `sort=<名前>&order=desc` 形式:

| ドロップダウン表示 | 生成パラメータ | Dialect概念 |
|---|---|---|
| ニコニコで人気 | `sort=hotLikeAndMylist&order=desc` | (site既定。指定なし=auto が事実上これ) |
| あなたへのおすすめ | (パーソナライズ) | 非対応(アカウント履歴依存で共有リンク化に不適) |
| 投稿日時 | `sort=registeredAt&order=desc` | new |
| 再生数 | `sort=viewCount&order=desc` | top(=bilibili top=click/最多播放と揃う「再生数=人気」) |
| コメント日時 | `sort=lastCommentTime&order=desc` | **commentDate(新規)**。直近にコメントが付いた順(コメント数=commentCountとは別概念)。2026-07-09にドロップダウンを実クリックして生成URLで確認 |
| いいね！数 | `sort=likeCount&order=desc` | likes |
| コメント数 | `sort=commentCount&order=desc` | comments |
| マイリスト登録数 | `sort=mylistCount&order=desc` | favorites |

**「再生時間」は並び順ドロップダウンに存在しない(2026-07-09再点検で訂正)**: 当初この節は「再生時間ソートが未採取」と記載していたが、実機でドロップダウンを開き直すと選択肢は上表の8個のみで、再生時間に相当する並び順は無い。「再生時間」という名前が付くのは下記の**検索フィルタモーダル**(`l_range=`)の方で、これは並び順ではなく既存のvideoLength概念(5分以内/20分以上)としてすでに実装済み。以前の記載は両者を混同していた。

- **旧コードは今も有効なエイリアス**: `sort=h` を開くと並び順ラベルが「ニコニコで人気」・人気順の結果、`sort=f&order=d` を開くと「投稿日時」・新着結果になることを確認。現行実装の new/top は壊れていない。ただしサイトが今生成するのは modern 形式なので実装も modern へ揃える([[feedback-migration-full-consistency]]の「実プロダクトがどうなっているかで判定」)。
- **URLに並び順が無いと状態が永続化される**: `/search/{q}`(パラメータ無し)を開くと直前に選んだ並び順(例:マイリスト登録数)が保持された=既定ソートはサイト/アカウント任せという既存コメントの裏付け。実装が常に sort を明示するのは正しい。
- **検索フィルタモーダル(GUI操作で採取)**: 4列。
  - **動画種別**: 指定なし / ユーザー(`kind=user`) / チャンネル(`kind=channel`)。**新フィルタ**=ユーザー投稿か公式チャンネル動画かで絞る。
  - **再生時間**: 指定なし / 5分以内(`l_range=1`) / 20分以上(`l_range=2`)。medium(ふつう)は存在しない=現行実装が medium を落とすのは正しい。
  - **投稿日時**: 相対プリセット(1時間以内/24時間以内/1週間以内/1ヶ月以内/1年以内)＋「日付を指定」で任意期間(`start=YYYY-MM-DD&end=YYYY-MM-DD`)。現行の start/end と一致。
  - **ジャンル**: 2026-07-08に採取済み(17値)。
- 全指定の合成例: `?sort=registeredAt&order=desc&start=2026-07-01&end=2026-07-07&kind=user&l_range=1`

#### niconico動画: イラスト観点再監査(2026-07-13、#70)

- 上記2026-07-09監査(並び順8値・検索フィルタモーダル4列・ジャンル17値)を照合基準に、GUI操作で検索フィルタボタンの状態変化を再確認しつつ、AI生成除外に相当する値が無いことを確認(ジャンル17値にAI関連カテゴリなし)
- 検索結果に表示される「センシティブな内容が含まれている可能性のあるコンテンツです / 設定を変更」の変更先は`account.nicovideo.jp/my/account`(アカウント設定)で、URLパラメータではなくアカウント永続設定。年齢区分の横断概念としてはFANBOX/YouTubeと同じ理由で対象外。ただし`genre=r18`(既存実装済み、ジャンル「例のソレ」)は年齢区分に近い絞り込みとして既に使える
- 人気絞り込みは`top`(再生数)/`likes`/`favorites`/`comments`/`commentDate`まで実装済みで既に手厚い。一次/二次創作タグ・画像形状/解像度・制作ツールの概念はniconico動画(動画プラットフォーム)の検索UIに存在しない
- 結論: niconico動画に新規フィールドは無し(既存実装のまま)

### Bluesky(bsky.app、GUI操作・ログイン済み)

- **言語フィルタがGUIドロップダウン化**: 検索ページの「すべての言語 ▾」で日本語を選ぶと `&lang=ja` の**URLパラメータ**を生成(現行実装はクエリ内 `lang:` 演算子)。GUIは `lang:ja` をクエリに入れても言語フィルタとして認識しない(ドロップダウンは「すべての言語」のまま=クエリ文字扱い)。ただし `lang:` 演算子自体は効く(`猫 lang:ja` と `猫 lang:en` でトップタブの先頭投稿が入れ替わることを確認)。→ 実装は実プロダクトが生成する `&lang=` 形へ寄せる。
- **タブ**: トップ(既定=top) / 最新(`tab=latest`=new) / ユーザー / フィード。現行の top/new と一致。
- **構造化フィルタ(`author=`/`media=` 等)はこのアカウント(ログイン済み)でも未露出**=まだゲート中。有効化されたら再採取が要る(監査候補としては解消、将来の変化点として台帳に注記)。

### note(note.com、GUI操作・未ログイン)

- **並び順チップ(GUI操作で採取)**: 人気(`sort=popular`=top) / 急上昇(`sort=hot`=hot) / 新着(`sort=new`=new)。3値とも現行実装と一致。
- **有料フィルタ**: 「すべての記事 ▾」セレクト(すべての記事=false / 有料の記事=true)。有料を選ぶと `context=note` → **`context=note_for_sale`** に切り替わり、全結果が有料記事(価格表示)に絞られることを実機確認。**新フィルタ(note専用「有料のみ」概念)**。並び順とも合成可(`context=note_for_sale&q=猫&sort=hot`)。
- **contextタブ**: 記事(note) / マガジン(magazine) / クリエイター(user) / メンバーシップ(circle)。現行実装は記事(context=note)のみ。他は「記事を探す」とは別対象(マガジン・クリエイターを探す)なのでキーワード記事検索の枠では対象外。
- 左サイドバーのカテゴリ(くらし/社会・経済 等)はカテゴリ閲覧ページで検索クエリを保持しない=検索フィルタではない。

### 実装方針(網羅実装・採否選定しない)

上記で実在確認したURLパラメータは見送り候補を作らず実装する(CLAUDE.md「フィールドの採否選定はしない」):

1. **niconico並び順**: modern形式へ移行し、top→再生数(viewCount、bilibili top=click と揃える)、comments/likes/favorites を追加マップ。
2. **niconico 動画種別**(`kind=user`/`channel`): 新概念。
3. **note 有料のみ**(`context=note_for_sale`): 新概念。
4. **Bluesky 言語**: `lang:` 演算子 → `&lang=` パラメータ。
5. **niconico コメント日時**(`sort=lastCommentTime`): 2026-07-09に並び順ドロップダウンを再度GUI操作で開き `sort=lastCommentTime&order=desc` を採取、新概念commentDateとして実装完了(「再生時間ソート」は上記の通り並び順に存在しないと判明したため対象外)。

### 探す種類(resultType)の水平展開(2026-07-09、GUI操作)

上記1回目監査の完了後、同日中に3サイトを再度掘り下げたところ、いずれも「トップタブ以外の検索結果タブ」に既存の`resultType`概念(Reddit/Pinterest/Twitch/YouTube/bilibiliが採用済み)へ横展開できる未収録のタブが見つかった。加えてBlueskyの`media=`について「ゲート中」という1回目監査の結論が誤りだったと判明した。

#### Bluesky: `media=true`のゲート解除・`tab=user`

- **`media=true`は既にゲート解除されている(URL叩きで確認)**: 検索フォームに対応するトグルはまだ無いが、`?q=猫&media=true`を開くとユーザー/フィードタブが非表示になり(サーバー側がパラメータを認識している証拠)、結果も画像・動画付き投稿のみになる。1回目監査の「まだゲート中」は誤りだったので訂正し、`mediaOnly`を`none`→`full`へ格上げした。GUIトグルが無いため出所は「URL叩き」(将来トグルが追加されたらGUI操作で採り直す)。
- **`tab=user`(アカウント検索)**: 検索ページの「ユーザー」タブを実際にクリックし、結果がアカウント一覧に切り替わることを確認(GUI操作)。さらに`q=猫 -犬`でアカウント検索すると「犬」を含むアカウントも普通に返る(除外が効かない)、引用符で囲んでも同じ結果(完全一致にならない)ことも確認。よって`resultType: 'people'`選択時は本文演算子(除外・完全一致・送信者・メンション先・リンク先・タグ・期間・言語・並び順)をすべて非対応に落とし、語句はそのまま連結するだけにした。

#### note: `context=user`(クリエイター)・`context=magazine`(マガジン)

検索結果タブは「記事/マガジン/クリエイター/メンバーシップ」の4つ。既存実装は「記事」(`context=note`)のみだったが、他3つも実際にGUI操作で開き直した:

- **クリエイター**(`context=user`): アカウント名・自己紹介にキーワードが一致する投稿者一覧。並び順チップ・`from:`演算子は無い。`resultType: 'people'`として実装。
- **マガジン**(`context=magazine`): 記事をまとめた企画の一覧(niconicoのシリーズと同種の「コンテンツの集合」)。並び順チップ・有料フィルタは無い。`resultType: 'series'`として実装(このセッションで新設した値)。
- **メンバーシップ**(`context=circle`): 有料コミュニティの一覧。他3つと違う独自の対象(サブスク課金コミュニティ)のため`resultType: 'circle'`として新設(2026-07-09 GUI操作で実装)。`from:`演算子は0件になり効かない(none)。有料のみは全件がそもそも有料のため意味を持たない(none)。並び順チップは「人気(既定・`sort=popular`)/新着(`sort=new`)」の2値のみ(急上昇は無い)で、クリエイター・マガジンと異なりこの2値だけは実装済み。

#### niconico: 検索結果タブ「ショート/シリーズ/マイリスト/ユーザー」

`/search/{q}`(動画本体)の他に、同じ検索フォームから4つのタブで切り替わる別ページがあると判明(いずれもGUI操作でタブをクリックしhrefを採取):

| タブ | パス | 実装 |
|---|---|---|
| ショート | `/search_shorts/{q}` | `resultType: 'short'`。並び順ドロップダウン(`sort=viewCount&order=desc`等)・フィルタパネルが動画本体と完全に同じ選択肢・パラメータを生成することを確認したため、genre/kind/videoLength/period/sortOrderを丸ごと共有 |
| シリーズ | `/series_search/{q}` | `resultType: 'series'`。専用の並び順(登録動画数/作成日/動画追加日時)を持つがフィルタパネルは無い |
| マイリスト | `/mylist_search/{q}` | `resultType: 'playlist'`(YouTubeの再生リストと共用)。シリーズと同じ並び順パラメータを共有(GUI操作で確認) |
| ユーザー | `/user_search/{q}` | `resultType: 'people'`。並び順は`sort=followerCount`/`videoCount`/`liveCount`(各`&order=desc`、フォロワー数/投稿動画数/生放送番組数) |

シリーズ/マイリスト/ユーザーはいずれもフィルタパネルが無く、ジャンル・動画種別・再生時間・期間のいずれも効かない(選ぶと「使えない」に落とす)。タグページ(`/tag/`)も動画本体専用なので、これらのタブではハッシュタグ語もただのキーワードとして扱われる。

**シリーズ/マイリスト/ユーザーの専用並び順は2026-07-09にGUI操作で採取・実装完了**(残タスクとして記録していたものを同日中に着手・解消)。ドロップダウンを実操作して以下を確認:

- **シリーズ**(`/series_search/{q}`。ドロップダウン4値): 「ニコニコで人気」(既定・明示選択でも`sort=_hotTotalScore&order=desc`という特殊値を送るだけで無指定時と一致するため指定なし(auto)に畳み込み)/「登録動画数」(`sort=videoCount`)/「作成日」(`sort=startTime`。新概念を増やさずSortOrderの`new`=新しい順に流用)/「動画追加日時」(`sort=lastAddedTime`。新概念`videoAdded`)。asc/desc切替アイコンもあるが既存の他サイトと同様descのみ実装(Dialectに昇順選択肢は無い)
- **マイリスト**(`/mylist_search/{q}`): シリーズと同一UIコンポーネントで、`videoCount`/`startTime`をGUI操作で個別に再確認(同一パラメータ名と確定)。`lastAddedTime`も個別に確認
- **ユーザー**(`/user_search/{q}`。ドロップダウン4値): 「あなたへのおすすめ」(`sort=_personalized`。ログイン依存の個人化並び順だが、未訪問のクエリで無指定時の既定と一致することを確認したため指定なし(auto)に畳み込み)/「フォロワー数」(`sort=followerCount`。新概念`followerCount`)/「投稿動画数」(`sort=videoCount`。シリーズ/マイリストの「動画の数」と同じ概念を共用)/「生放送番組数」(`sort=liveCount`。新概念`liveCount`)

新規追加したSortOrder値は`videoCount`/`videoAdded`/`followerCount`/`liveCount`の4つ(`new`は既存概念を流用)。niconico専用でありlimitSortにより他サイト・他resultType(動画/ショート)では非対応に落とす。lint/build/check:operators/check:urls通過、Dialect実機(dev server)で各生成URLを実際に開いて並び順ドロップダウンの表示・結果順が一致することを確認。noteのメンバーシップは2026-07-09に`resultType: 'circle'`として実装済み(上記参照)。

# OR対応 GUI裏取り(2026-07-11)

「このどれかを含む」箱(スコープ限定OR・入れ子なし・OR演算子を表に出さない案)を将来実装するかどうかの判断材料として、OR撤去前の実測記録([[additive-narrowing-principle]]参照、OR自体は現在Dialectの思想として全廃済み)を全20サイトぶんGUI操作で裏取りした。**本節作成時点(2026-07-11)は調査のみだったが、同日中にissue #26として`keywordsOr`概念を実装した(型/6サイトのbuildParts・parseUrl/UI/i18n/check:operators・check:parts・check:reverse・docs/operator-checklist.mdの該当行まで一式)**。「足す=絞る原則」への意識的な例外として導入している。

対象は次の2グループ:
- **OR対応の可能性が高い6サイト**(過去の記録がURL叩き/件数比較/doc中心だったため、検索ボックス・詳細検索フォームを実際に操作して正確なOR構文を採取)
- **OR記録が無く未確認だった10サイト**(検索フォーム・詳細検索/フィルタパネルを実際に開き、「いずれかを含む」相当の入力欄が無いこと、および`OR`をクエリに書いても機能しないことを確認)

確定済み非対応4サイト(Misskey・note・あにまん・5ch)は今回対象外(既存記録で十分)。

## OR対応6サイトの確定構文

いずれも検索ボックス・詳細検索フォームに実入力して送信し、生成URLと結果件数/内容を確認した(**出所: GUI操作、2026-07-11**)。3つの流派に分かれる。

### 流派1: 大文字`OR`+`()`グループ(X・Reddit・pixiv)

| サイト | 検証内容 | 生成される`q`/クエリ | 備考 |
|---|---|---|---|
| X | `x.com/search-advanced`の「次のキーワードのいずれかを含む」欄に`猫 犬`と入力→検索 | `(猫 OR 犬)` | 3語(`猫 犬 鳥`)入力で`(猫 OR 犬 OR 鳥)`(単一グループ内に複数OR)。「すべて含む」欄に`手芸`、「いずれか」欄に`猫 犬`を併用すると`手芸 (猫 OR 犬)`(AND語+OR括弧グループ)。要ログイン(ログイン済みChromeで確認) |
| Reddit | `reddit.com/search`の検索ボックスに直接`cats OR dogs`と入力 | `cats OR dogs`(2語なら括弧無しでも機能)、`(cats OR dogs) yarn`(AND語と併用時は括弧で明示) | 結果本文を確認、`(cats OR dogs) yarn`は全件yarn関連でcat/dogいずれかを含む正しいBoolean AND+OR動作を確認。ログイン不要 |
| pixiv | `pixiv.net`検索ボックスに`猫 OR 犬`と入力 | `猫 OR 犬`(括弧無しでも機能、`s_mode=tag&type=artwork`) | タイトルが「#猫 OR 犬のイラスト・マンガ作品(100万件超)」となり、猫単独(10万件超)より大幅増=union と確認。AND語併用は`手芸 (猫 OR 犬)`(括弧付き)。ログイン済みChromeで確認(既存記録通り未ログインでも動作するはずだが今回はログイン状態で確認) |

### 流派2: `|`(パイプ)区切り(YouTube)

| サイト | 検証内容 | 生成されるクエリ | 備考 |
|---|---|---|---|
| YouTube | `youtube.com`検索ボックスに`猫 \| 犬`と入力 | `search_query=猫+%7C+犬`(`\|`は`%7C`) | 結果に猫動画・犬動画が混在(「犬と猫の絆」「猫と犬の熾烈な戦い」等)し union と確認。ログイン不要 |

### 流派3: 括弧禁止・並置(niconico動画・niconico静画)

| サイト | 検証内容 | 生成されるURL/件数 | 備考 |
|---|---|---|---|
| niconico動画 | `nicovideo.jp`検索ボックスに`猫 OR 犬`と入力→検索 | `/search/猫%20OR%20犬` → 542,896件 | `(猫 OR 犬)`(括弧付き)は11,103件に激減=括弧はリテラル文字として扱われ検索が壊れる(2026-07-02のURL叩き記録を検索ボックス経由のGUI操作で再確認・一致) |
| niconico静画 | `seiga.nicovideo.jp`検索ボックスに`猫 OR 犬`と入力→検索ボタン押下 | `/search/猫%20OR%20犬?target=illust` → 106,374件(猫単独73,597件+犬単独34,367件の合算に近い=union) | `(猫 OR 犬)`は1,232件に激減=動画と同じく括弧はリテラル扱い。この文法が静画側でGUI操作により裏取りされたのは今回が初めて(従来は「件数比較」=URL叩きのみ) |

**結論**: niconicoの2サービス(動画・静画)は共通のsnapshot検索基盤を使っており、文法(括弧禁止・スペース区切りOR・並置で複数グループ)が完全に一致することを両方でGUI操作により確認した。

## OR非対応10サイトの確認

いずれも検索フォーム・詳細検索/フィルタパネルを実際に開き、(a)「いずれかを含む」に相当する入力欄が無いこと、(b)クエリに`OR`と書いても文字列として扱われるだけでunion検索にならないこと、の両方を確認した(**出所: GUI操作、2026-07-11**)。

| サイト | 確認方法 | 結果 |
|---|---|---|
| Bluesky | ログイン済みChromeで「高度な検索オプションを開く」ボタン(**新発見**、後述)→フィルターモーダルの全項目を確認。検索ボックスにも`猫 OR 犬`を試行 | モーダルは「これらの単語すべて」(AND)/「これらの単語をすべて含まない」(除外)/「正確なフレーズ」/日付/言語/メディア/含める/通知対象/追加フィルタ(含める・除外するの2択のみ)で構成、「いずれかの単語」に相当する項目は無い。OR非対応と確定 |
| Mastodon(mastodon.social) | ログイン済みChromeで検索ボックスに`猫 OR 犬`と入力 | 結果はユーザー名に`Or`を含むアカウント(`Trip Or Treats`)や本文に"or"を含む投稿がヒットしただけで、Meilisearchの全文一致が`OR`を通常の3文字語として扱っているだけと確認。OR非対応 |
| Instagram | ログイン済みChromeで検索アイコン→検索ボックスに`猫 OR 犬`と入力 | 表示されたのはアカウント名のタイプアヘッド候補のみ(「オリバ犬猫病院」等)で、Enterを押しても専用の検索結果ページに遷移しない。そもそもキーワード全文検索のUIが無く、OR概念が入る余地が無いことを再確認 |
| Twitch | 検索ボックスに`猫 OR 犬`と入力→検索 | 結果はタイトルに"or"を含む配信者(`!hello or HeyGuys`)やカテゴリ名に猫・犬両方の文字が偶然含まれるもの(「ライムの決闘：猫と犬のギャンブルの戦い」)のみで、いずれもリテラル一致。OR非対応 |
| Tumblr | 「詳細検索」パネルの全項目を確認。「これらすべての単語」欄に`猫 OR 犬`と入力 | パネル構成は「これらすべての単語」(AND)/「これらすべてのタグ」(AND)/「単語またはタグを除外」/「ブログから」/「コミュニティ内」/日にちの範囲、「いずれかの単語」欄は無い。クエリのプレビューは入力をそのまま反映するのみでOR特別扱いなし。OR非対応 |
| はてなブックマーク | 検索ボックスに`猫 OR 犬`と入力→検索 | 猫単独49,705件に対し、結果は猫・犬と無関係な低順位記事のみでunionになっていない=`OR`をただの3文字目のAND対象として扱っている。OR非対応 |
| FANBOX | 検索ボックスに`猫 OR 犬`と入力→検索 | `type=creator&q=猫 OR 犬`(クリエイター名のあいまい検索)に遷移するのみ。投稿の全文検索URL自体が存在せず(単一タグページ`/tags/{タグ}`のみ)、OR概念が入る構造的余地が無いと再確認 |
| Fantia | ログイン済みで投稿検索の検索ボックスに`猫 OR 犬`と入力→検索 | `keyword=猫 OR 犬&keyword_type=title_only`で**0件**(「OR」を含む文字列がタイトルに存在しないため)。既知の除外・完全一致と同じくリテラル一致のみで、OR非対応 |
| bilibili | 検索ボックスに`猫 OR 犬`と入力→検索 | 結果はタイトルに"or"を含む動画(「猫系or犬系？」等)のみでunionになっていない。「更多筛选」パネルにもOR相当の項目は無い(既存記録通り日期範囲・時長のみ)。OR非対応 |
| Pinterest | 検索ボックスに`猫 OR 犬`と入力→検索 | `q=猫 OR 犬`は通常のあいまい検索(ML類似度ベース)に流れるだけで、フィルターパネルも4択(すべてのピン/動画/ボード/プロフィール)のみ。既存記録通りBoolean演算子自体が存在せずOR非対応 |

### 新発見: Blueskyの高度な検索オプションモーダル(2026-07-11)

既存記録(2026-07-09)では「構造化フィルタパラメータ(`author=`/`media=true`等)はゲート中」としていたが、今回**検索ページに「高度な検索オプションを開く」ボタンとフィルターモーダルGUIが新規に追加されている**ことを発見した(2026-06〜07の間に実装されたとみられる)。モーダルは「これらの単語すべて」「含まない」「正確なフレーズ」「日付範囲」「言語」「メディア」「含める(投稿と返信)」「通知対象」に加え、「+フィルターを追加」で「含める/除外する」×「これらの人から/ドメイン」の複数行フィルタを追加できる。これは柱B(フィールド機構)の観点で見送り・非対応と判定していた項目の一部がGUI経由で見えるようになった可能性があるため、**別途あらためて全項目のGUI監査が必要**([[dialect-backlog]]への追記候補、本タスクのスコープ外につき今回は実装しない)。

### 新発見: Blueskyがログアウト時検索不可に変更(2026-07-11)

既存記録(2026-07-09時点)は「ログイン不要(実測確認済)」だったが、in-appブラウザ(未ログイン)で`bsky.app/search?q=猫`を開いたところ「ログアウト時の検索は現在利用できません。サインインするかアカウントを作成してください」と表示され、**検索結果が一切表示されなくなっていた**。ログイン済みChromeでは従来通り機能する。Blueskyがログアウト時検索を仕様変更で無効化した可能性が高い。**この変更はOR調査と直接関係しないが、[[dialect-backlog]]のBluesky「ログイン不要」記載を更新する必要がある**(本タスクは調査のみのためコード変更はしていない)。

## 結論サマリ

### OR(「いずれかを含む」)を実行できるサイトと構文

| サイト | 構文 | 複数語グループ化 | AND語との併用 |
|---|---|---|---|
| X | `(a OR b)` (大文字OR必須) | 単一グループに`OR`を連ねる: `(a OR b OR c)` | `word (a OR b)` |
| Reddit | `a OR b` (2語なら括弧省略可) | `(a OR b OR c)`(公式Boolean+括弧) | `word (a OR b)` |
| pixiv | `a OR b` (括弧省略可) | `a (b OR c)`(公式ヘルプ記載の複数グループ表現) | `word (a OR b)` |
| YouTube | `a \| b` (パイプ、`%7C`) | 未確認(pipeの複数使用は今回未検証) | `word a\|b`(2026-07-11に実装時、youtube.comの検索ボックスで「手芸 猫\|犬」を検索し、猫関連・犬関連の動画が両方混在する=union AND併用が成立することをGUI操作で実測) |
| niconico動画 | `a OR b` (**括弧禁止**、使うとリテラル文字扱いで検索が壊れる) | 並置: `a OR b c OR d`(ORはスペースANDより強く結合) | `a OR b word`(OR連鎖の右にAND語を続ける) |
| niconico静画 | `a OR b` (**括弧禁止**、動画と同一挙動) | 動画と同じ並置パターンと推定(未実測だが同一基盤のため高信頼) | 動画と同様と推定 |

### スコープ限定OR実装時のper-siteシリアライズ要件(判断材料。2026-07-11に下記の通り実装済み)

「このどれかを含む」箱(キーワード次元だけ・入れ子なし)を実装する場合、6サイトが3つの異なる直列化ロジックを要求する:

1. **括弧+OR派(X/Reddit/pixiv)**: キーワード群を`(w1 OR w2 OR ...)`として1グループにまとめ、他の通常キーワードとスペースAND結合する。実装コストは低い(既存keywordsのシリアライズに1段ラップを足すだけ)
2. **パイプ派(YouTube)**: キーワード群を`w1|w2|...`として結合。通常キーワードとの併用は2026-07-11の実装時にGUI操作で追加調査し、`word w1|w2`の順で問題なく成立することを確認(上記マトリクス参照)
3. **括弧禁止・並置派(niconico動画/静画)**: 括弧を送ってはいけない、という他サイトと逆方向の制約がある。キーワード群をそのまま`w1 OR w2`として並べ、他の通常キーワードは末尾に続ける(`w1 OR w2 keyword`)。ORの右側にAND語を続けると自動的に`(w1 OR w2) AND keyword`相当になる特殊な結合規則があるため、通常キーワードとの共存位置に注意が必要

いずれのサイトも「1個の値だけを選んだ場合はOR構文を送らず通常のkeywordsと同じ扱いにする」実装が自然(1語だけでは`(w1 OR)`のような壊れた構文にならないようにする配慮が必要)。

残り14サイト(既存の確定非対応4サイト含む)はOR相当の機能が無いため、スコープ限定OR機能はこれら6サイトのみでの部分対応となる。この非対称性(20サイト中6サイトのみ対応)がUX上どう見えるかは、実装可否の判断材料としてユーザーへ差し戻す。

## Bluesky 高度な検索オプションモーダル 全項目監査(2026-07-11、GUI操作・ログイン済み・issue #27)

上記「新発見: Blueskyの高度な検索オプションモーダル」節で存在だけ確認し先送りにしていた全項目を、実際にモーダルの各コントロールを操作して生成URLを採取した(**出所: GUI操作**)。ログイン済みChrome(claude-in-chrome、実セッション)で`bsky.app/search?q=<語>`→「フィルター」ボタン→モーダルの各欄を1つずつ操作して`検索`を押し、遷移後のURLを確認する形で全項目を回った。

### ログイン要否(item 1)

未ログイン(in-appブラウザ)で`bsky.app/search?q=猫`を開くと「ログアウト時の検索は現在利用できません。サインインするかアカウントを作成してください」と表示され結果が一切出ない一方、ログイン済みChromeでは同URLが正常に動作することを突き合わせて確認した。**Blueskyは`requiresLogin: true`に変更する**(既存の「ログイン不要」記載・`docs/operator-checklist.md`・READMEは陳腐化のため更新)。ディープリンク自体はログイン済みユーザーには有効なので、UIには他のログイン必須サイトと同じ注記を出すに留める。

### モーダル全項目の採取結果(item 2)

既存概念とほぼ1:1で対応する項目(「これらの単語すべて」=keywords、「これらの単語をすべて含まない」=exclude、「正確なフレーズ」=exactPhrase、「次の日付以降/以前」=period、「言語」=language)は生成パラメータが既存実装(`q=`内の語・`lang=`)と一致し新発見なし。以下は新発見または既存記録の訂正:

| モーダル項目 | 生成パラメータ | 備考 |
|---|---|---|
| メディア→「動画付きの投稿のみ」 | `video=true` | **新規**。既存の`media=true`(画像+動画)とは別の絞り込み。3値のうちの1つとしてGUIドロップダウンに表示 |
| 含める→「返信を含まない」 | `replies=none` | **新規概念**。投稿のみ(返信を除く) |
| 含める→「返信のみ」 | `replies=only` | **新規概念**。返信のみに絞る(逆方向) |
| 通知対象→「フォロー中の人」 | `following=true` | **新規概念**。閲覧アカウントがフォローしている相手の投稿のみに絞る(ビューア依存だが実在するURLパラメータのため実装対象) |
| 追加フィルタ→「これらの人から」(含める/除外する) | `author=`/`excludeAuthor=`(スペース区切り複数、**OR**) | 既存`fromUser`(単一値・q内`from:`トークン)とは別径路。`author=jay.bsky.team+pfrazee.com`で両アカウントの投稿が混在して返ることを確認し複数値=OR(いずれかに一致)と確定。除外は既存`excludeUser`と同じ「除外対象を空白区切りで複数」という意味論に合致 |
| 追加フィルタ→「これらの人にメンション」(含める/除外する) | `mentions=`/`excludeMentions=`(スペース区切り複数、OR) | 既存`mentionsUser`(単一値・q内`mentions:`トークン)とは別径路 |
| 追加フィルタ→「これらのドメイン」(含める/除外する) | `domain=`/`excludeDomain=`(スペース区切り複数、OR) | 既存`domain`(単一値・q内`domain:`トークン)とは別径路 |
| 追加フィルタ→「これらのURL」(含める/除外する) | `url=`/`excludeUrl=` | **完全新規概念**。埋め込みリンク・カード先のURLで絞り込む(dialectに対応する既存概念なし) |
| 追加フィルタ→「これらのハッシュタグ」(含める/除外する) | `tag=`/`excludeTag=`(スペース区切り複数) | **OR意味論**。`tag=cat+dog`で結果の大半が`#cat`のみ(`#dog`を伴わない)投稿だったことを確認し、既存の`hashtag`概念(空白区切り複数=**AND**、q内`#tag`トークンの並置)とは意味論が異なる別物と確定 |

`author=`/`mentions=`/`domain=`/`tag=`は「新パラメータ形式で複数値のOR検索ができる」という、既存のq内トークン方式(単一値または`hashtag`のAND)にはない新機能。既存フィールドを流用するかどうかはコード実装時の設計判断とする(このドキュメントは事実採取のみ)。

### 実装方針

CLAUDE.md「フィールドの採否選定はしない」に従い、上記で実在確認したパラメータはすべて実装対象とする。

### Bluesky: イラスト観点再監査(2026-07-13、#70。推論・新規ログイン無し)

- 2026-07-11(2日前)に「高度な検索オプション」モーダルの全項目をGUI操作・ログイン済みで採取済み(上記)。この監査は非常に新しく、モーダルの全コントロールを1つずつ操作する形で網羅済みのため、今回はXと同じ理由(繰り返しのログイン依頼を避ける判断)で新規ログインは行わず、この既存監査結果をイラスト観点の各項目に照合する形で推論確定した
- モーダル全項目(メディア種別・返信/フォロー中限定・追加フィルタ群author/mentions/domain/url/tag)はいずれも投稿の属性(発信者・埋め込みリンク・ハッシュタグ・メディア有無)を扱うもので、pixiv的な作品分類概念(AI生成除外・年齢区分・一次/二次創作タグ・画像形状・制作ツール)に相当する項目は無い
- 結論: Blueskyに新規フィールドは無し(既存実装のまま)

## GitHub / Qiita / Zenn 調査(2026-07-12、issue #53〜#55)

技術者クラスタ3サイトの調査。3サイトともログイン状態依存の差分を確認する必要があり、いずれも「未ログインで検索・演算子自体は完結する」ことを確認した(未ログインのin-appブラウザで検証。GitHub/Qiitaはアカウント保有前提でログイン差分の切り分けも行った)。

### GitHub(**出所: GUI操作**、`github.com/search/advanced` の詳細検索フォームへ実際に値を入力し、生成された `q=` 文字列を採取。2026-07-12)

`/search/advanced` は入力するそばから画面下の隠しクエリ入力(`input[name=q]`)へリアルタイムでqualifierを合成する構造。各欄に値を入れて実際に合成された文字列を採取した(フォーム送信もして最終URLが `github.com/search?q=<query>&type=<Type>` になることも確認)。

- **探す種類(resultType)**: 検索結果ページ上部のタブが `type=repositories` / `code` / `issues` / `pullrequests` / `discussions` / `users` に対応(未ログインで確認)
- **並び順**: `s=stars&o=desc` で「Most stars」表示を確認(リポジトリ検索)。他の探す種類でも `s=`/`o=` が使えると見込まれるが今回はリポジトリのみ実機確認
- 詳細検索フォームで確認できたqualifier(いずれも同フォームの対応欄に入力し `q=` 内へ実際に合成されることを確認):
  - リポジトリ: `user:` (from these owners) ・`repo:owner/name`(in these repositories)・`created:>YYYY-MM-DD`(作成日)・`stars:>100` / `10..50`型の範囲構文・`forks:`・`size:`(KB)・`pushed:<YYYY-MM-DD`・`language:TypeScript`・`license:mit`(SPDX ID)・`fork:true/only`(フォークを含む/のみ)
  - コード検索: 「拡張子」「パス」「ファイル名」の3欄はいずれも内部的に **`path:` のglobパターンへ統一されている**(`extension:rb` ではなく `path:*.rb`、`filename:app.rb` ではなく `path:**/app.rb` と合成されることをGUI操作で確認。2023年のコード検索刷新で `extension:`/`filename:` は廃止され `path:` に一本化された模様。学習知識の「extension:」を鵜呑みにせずGUI確認したことで判明)
  - Issue検索: `state:open/closed`・`reason:completed/"not planned"/reopened`(state_reason)・`comments:>5`・`label:bug label:"help wanted"`(複数指定=複数qualifier列挙、スペース含む値は要引用符)・`author:`・`mentions:`・`assignee:`・`updated:<YYYY-MM-DD`
  - ユーザー検索: `fullname:"Grace Hopper"`(要引用符)・`location:`・`followers:>100`・`repos:>10`(公開リポジトリ数)・`language:`(Working in this language、リポジトリと同じ言語プルダウン)
  - 検索結果サイドバーのファセットから追加確認: `is:public` / `is:private`(未ログインでもリンクとして存在)
- **doc補完**(GUI未確認・公式ドキュメント記載): `topic:`・`in:name,description,readme`・`archived:true/false`・`mirror:true/false`・`org:`。これらはGUIの詳細検索フォーム自体には専用欄が無く(サイドバーの折りたたみファセットの奥にある可能性はあるが未展開)、qualifierとして直接入力する形式のみ確認できている
- **ログイン要否**: 詳細検索フォーム・検索結果とも未ログインで完全に機能する。GitHubアカウント自体は開発者(ユーザー)が保有しているが、今回の調査項目(検索フォーム操作)はログイン差分が想定しにくい種類(検索対象がpublicリポジトリ中心)のため、未ログインのみで「ログイン不要」と判定する
- **実装方針(コスト分割)**: 演算子点数が既存サイト中最多(全部で約20種)なため、今回のPRでは以下のみをPhase 1として実装する: resultType(6種)・keywords/exactPhrase/exclude・`fromUser`→resultTypeに応じ`user:`(リポジトリ/コード)or`author:`(Issue/PR)へ翻訳・`period`→`created:`・新規概念`codeLanguage`(`language:`、自由入力)・新規概念`minStars`(`stars:`、数値文字列)。残り(forks/size/pushed/license/fork inclusion/path系/comments/labels/mentions/assignee/updated/fullname/location/followers/repos/state/reason/is:public,private/topic/archived等)は実装コストが大きい独立した塊のため、別issueとして切り出す(「これは載せない」という取捨選択ではなく、CLAUDE.md掲載の「実装コストのぶんは分割してよい」に基づく作業分割。今回のGUI操作記録は上記の通りここに残したので、次の着手時に再調査は不要)
- **補記(出所: doc)**: `period`(`created:`)・`minStars`(`stars:`)の範囲構文は、GUI操作では単方向(`created:>YYYY-MM-DD`・`stars:>100`)のみ実測した。実装では期間の両端指定(`created:{since}..{until}`)・以上/以下(`created:>=`/`stars:>=`)も使うが、これらは公式ドキュメント(docs.github.com/search-syntax)記載の標準構文であり、GUI操作そのものでは範囲構文の全パターンまでは踏んでいない

### GitHub 第2弾調査(**出所: GUI操作**、issue #64、2026-07-12)

`github.com/search/advanced` は全resultType分のフィールドを1ページに並べて表示する構造(タブ切り替えではない)。React管理下の入力のためJSから`HTMLInputElement.prototype`のネイティブsetterで値を設定し`input`/`change`イベントを発火、`.completed-query`(画面上のクエリプレビュー)の実テキストを採取する形でGUI操作を行った(通常のクリック操作と同じ結果になる、React controlled inputへの正規の値反映経路)。

- **重大な新発見: resultTypeは6種ではなく11種**。検索結果ページのタブ実測(`type=`URL)で確認: `code`・`repositories`・`issues`・`pullrequests`・`discussions`・`users`(以上Phase1既知)に加え、**`commits`・`registrypackages`・`wikis`・`topics`・`marketplace`が新たに存在**(`github.com/search?q=test&type=wikis`等で実際にタブ遷移・専用結果ページが表示されることを確認)。詳細検索フォーム(`/search/advanced`)にも「Wiki options」節(`updated:`のみ)が存在し対応。Issue #53・#64のいずれの起票時点でもこの5種は把握されておらず、今回のGUI操作で偶然発見した。**別issue(第3弾)へ切り出す**(この5種の演算子調査・実装は本issueのスコープ外)
- **forks:**(リポジトリ): `search_forks`欄、プレースホルダ`50..100, 200, <5`。実測`10..50`→`forks:10..50`(範囲構文そのままGUI確認済み)
- **size:**(リポジトリ): `search_size`欄、プレースホルダ「Repository size in KB」。実測`500`→`size:500`(KB単位の裸の数値)
- **pushed:**(リポジトリ): `search_push`欄、プレースホルダ`<YYYY-MM-DD`。実測`2024-01-01`→`pushed:2024-01-01`(入力した文字列がそのまま反映。`created:`と同じ日付演算子でありrange構文`..`/`>=`等はdoc確認=docs.github.com/search-syntaxに準拠と推定)
- **license:**: `search_license`セレクト、`optionCount:51`。「Any license」以外の全50件のvalue(SPDX相当ID)を実機取得済み: `0bsd, afl-3.0, agpl-3.0, apache-2.0, artistic-2.0, blueoak-1.0.0, bsd-2-clause, bsd-2-clause-patent, bsd-3-clause, bsd-3-clause-clear, bsd-4-clause, bsl-1.0, cc-by-4.0, cc-by-sa-4.0, cc0-1.0, cecill-2.1, cern-ohl-p-2.0, cern-ohl-s-2.0, cern-ohl-w-2.0, ecl-2.0, epl-1.0, epl-2.0, eupl-1.1, eupl-1.2, gfdl-1.3, gpl-2.0, gpl-3.0, isc, lgpl-2.1, lgpl-3.0, lppl-1.3c, mit, mit-0, mpl-2.0, ms-pl, ms-rl, mulanpsl-2.0, ncsa, odbl-1.0, ofl-1.1, osl-3.0, postgresql, unlicense, upl-1.0, vim, wtfpl, zlib, cc, gpl, lgpl`(表示名は各value横に付記、コードのSELECT_OPTIONS実装時に転記する)
- **fork:true/only**(フォーク包含): リポジトリ・コードそれぞれに専用セレクトが1つずつ存在(同じ3値)。value実測: `''`→表示"not"(除外・既定)、`'true'`→表示"and"(含む)、`'only'`→表示"only"(フォークのみ)。既存の`fromUser`と同様「リポジトリ・コードで共用」の1概念として実装可能
- **path:**(コード検索、既存記録の再確認): `search_extension`(拡張子)→`*.rb`型、`search_path`(パス)→入力そのまま、`search_filename`(ファイル名)→`**/app.rb`型。3欄それぞれ独立入力可、複数指定時は`path:`が複数回出現しAND(コード検索的に妥当)
- **state:**(Issue): `search_state`セレクト、値`''/open/closed`。実測`open`→`state:open`
- **reason:**(Issue): `search_state_reason`セレクト、値`''/completed/'not planned'/reopened`。実測`not planned`→`reason:"not planned"`(空白含む値は要引用符)
- **comments:**(Issue): `search_comments`欄、プレースホルダ`0..100, >442`(範囲構文対応)。実測`>5`→`comments:>5`
- **label:**(Issue): `search_labels`欄、カンマ区切り入力→**カンマ区切りの各値がそれぞれ独立した`label:`qualifierに展開**(実測`bug, help wanted`→`label:bug label:"help wanted"`)。空白を含む値のみ引用符。複数`label:`はAND(すべてのラベルを持つ)という既存`hashtag`概念と同じ意味論
- **author:/mentions:/assignee:**(Issue): `search_author`/`search_mention`/`search_assignment`欄、いずれもカンマ区切り→複数`qualifier:`へ展開(実測`octocat, hubot`→`author:octocat author:hubot`等)。`author:`は既存Phase1の`fromUser`と同一演算子のため新規概念は不要。`mentions:`は既存の汎用`mentionsUser`概念(単一値)を流用し複数値対応は見送る(既存概念の設計に合わせる。他サイトのmentionsUserも単一値のため一貫性を優先)。`assignee:`は複数人へのAND指定に実用上の意味があるため新規concept(chips、AND)として実装する
- **fullname:**(ユーザー): `search_full_name`欄。実測`Grace Hopper`→`fullname:"Grace Hopper"`(空白を含むため常に引用符)
- **location:**(ユーザー): `search_location`欄。**注意**: GUIはカンマを複数値の区切りとして扱うため、実測`San Francisco, CA`→`location:"San Francisco" location:CA`という意図しない分割になった(1人のユーザーの所在地は1つのはずでAND分割は実用上ほぼ意味をなさない)。dialect実装では複数値分割はせず、入力値全体を1つの`location:"…"`として常に引用符で包む単一値concept(GitHubの生GUIの癖をそのまま模倣しない、事実に基づく実装判断)
- **followers:/repos:**(ユーザー): `search_followers`/`search_public_repos`欄。プレースホルダは範囲構文対応(`20..50, >200, <2`/`0, <42, >5`)を示すが、既存の`minStars`等と同じ「単一値→`>=N`」表現に統一する(コードベース全体でmin系concept=`>=N`単一値という既存パターンに合わせる。他サイトのmin_faves等も同様に範囲を露出していない)
- **is:public/is:private**: advanced検索フォーム自体には専用欄が無く(Phase1調査時点の記録通りサイドバーファセットのみ)、今回のGUI操作でも確認できず。既存記録(サイドバーファセットで存在確認済み)を踏襲する
- **doc補完(未変更)**: `topic:`・`in:name,description,readme`・`archived:true/false`・`mirror:true/false`・`org:`は今回もGUI上に専用欄を発見できず(advanced検索フォームの全欄を実査済み)。引き続きdoc根拠のみで実装する
- **ログイン要否の訂正(出所: 実URL確認、2026-07-12)**: Phase1(issue #53)は「検索フォーム・検索結果とも未ログインで完全に機能する」と一括判定していたが、これは誤りだった。`type=code`(コード検索)の結果ページを実際に未ログインで開くと「Sign in to search code on GitHub」という案内のみが表示され、**コード検索の結果閲覧にはログインが必要**と判明した(`github.com/search?q=猫 path:*.rb&type=code`で実機確認)。URLの組み立て自体(qualifierの合成)は未ログインでも詳細検索フォームで可能なため、機能自体を欠くわけではない。pixivのR18(`note.pixiv.r18Login`)と同じ「partial+ログイン要の注記」として実装した(`resultType`概念のdynamicSupportで`state.resultType === 'code'`のときlevelを`partial`に、`note.github.codeLoginRequired`を付与)。他のresultType(repositories/issues/pullrequests/discussions/users)は今回も未ログインで結果閲覧まで確認できており、この訂正はコード検索のみに限定される

### Qiita(**出所: GUI操作+doc**。詳細検索パネル自体はログイン必須と判明したため、公式ヘルプ記載の演算子を検索ボックスへ直接入力し動作を確認する形で裏取り。2026-07-12)

- 検索結果ページの「検索オプションを表示」ボタン(tuneアイコン)をクリックすると、未ログイン状態では**ログイン/新規登録モーダルが開く**ことを確認(パネルの中身は見られない)。同じく「ストックのみ」トグルは未ログインだと `/login?...` へのリンクになっている。**つまりQiitaの詳細フィルタGUI・ストック機能はログイン必須**
- 一方、演算子そのものは未ログインでも `q=` へ直接入力すれば機能することを確認: `tag:Python 猫`(6348件→1128件に絞り込み)・`title:猫 stocks:>10`(52件)・`user:qiita created:>2024-01-01`(38件)。**演算子の実行自体はログイン不要**で、ログインが要るのはそれを組み立てる詳細フィルタGUIのみ、という切り分けになる
- **探す種類(resultType)**: 「記事」タブ=`/search?q=`、「質問」タブ=`/question-search?q=`(同じ演算子がそのまま使えることを確認)
- **並び順**: 検索結果の並び順メニューから3値を確認: `sort=rel`(関連順・既定)・`sort=created`(新着順)・`sort=stock`(ストック数順)
- **ストックのみ / 質問タブの「購読のみ」**: いずれもログイン必須のためGUI操作で裏取りできず、パラメータ名は未確認。ビューア依存の個人フィルタ(Blueskyの`following=true`と同種)と推定されるが、確証を得るには実アカウントでのログインが要る。**ユーザーに要相談**(本人によるログイン操作が必要な箇所)
- **実装方針**: keywords・tag:・title:(titleOnly)・user:(fromUser)・stocks:(min値、新規概念`minStocks`)・created:(period)・resultType(記事/質問)・sortOrder(3値)は今回のPRで実装する。ストックのみ/購読のみの2トグルはパラメータ未確定のため見送り、Issueに調査continuation注記を残す

### Zenn(**出所: GUI操作**、`zenn.dev/search` の各リンク・タブを実操作。2026-07-12)

- 検索欄・意味検索トグル・5タブ・3並び順: `q=`(キーワード)・`mode=semantic`(意味検索、BETA)・`source=articles/books/scraps/users/publications`(探す種類)・`order=daily/alltime/latest`(トレンド当日/全期間/最新)
- **表層で止めない失敗と訂正**: 検索語「猫」ではBooks/Publicationsの該当件数が0件のためタブ自体が表示されず、初回調査で見落として「3タブのみ(articles/scraps/users)」「source=booksは無視される」と誤って記録した。件数が出る語(Python)で再検証したところ、`source=books`・`source=publications`とも実在しURLが正しく保持されることを確認(タブ見出しの件数表示 Articles/Books/Scraps/Users/Publications で5種そろって出現)。**タブの見た目上の有無を「対応の有無」と早合点しないこと**の実例として記録する
- `-語`(除外)は非対応と確認: 「Python」単独は5939件(articles)だが「Python -Django」は**全カテゴリ0件**になった。除外が効くなら微減するはずが全滅したことから、`-`はただの文字として字句解析を壊すだけで除外機能自体が存在しないと判定
- トピック(タグ)ページ `/topics/{name}` は存在するが、`?q=` を付けても絞り込みに反映されない(件数が同じ)ことを確認したため、**キーワード検索とタグ絞り込みは組み合わせられない構造**と判明。既存の`hashtag`概念(他条件と組み合わせて使う前提)とは適合しないため、Zennでは実装対象外とする(取捨選択ではなく構造上組み合わせ不可という事実に基づく)
- **ログイン要否**: 検索UI内にログイン誘導要素(リンク・トグル)が一切無く、全機能が未ログインで完結すると確信できる(GUI操作で確認)。ログイン差分は無し
- **実装方針**: keywords(exclude/exactPhraseは非対応と確認済みにつき実装しない)・resultType(articles/books/scraps/users/publications の5値)・sortOrder(daily/alltime/latest)・新規概念`semanticSearch`(mode=semantic)を実装する
