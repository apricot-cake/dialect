...
- **メンバーシップ**(`context=circle`): 有料コミュニティの一覧。「人気」「新着」の並び順を持つ、他3つと違う独自の対象(サブスク課金コミュニティ)。適合する既存resultType値が無く、新しい値を割り当てる設計判断が要るため今回は見送り、実装コストの分割として残す。
...
3サイトとも「探す種類」の専用並び順(niconicoシリーズ/マイリスト/ユーザー、noteメンバーシップ)は未実装のまま残る。既存の[[field-mechanism-redesign]]同様、「採否選定はしない」方針のもと見送りではなく実装コストの分割として記録する。
