...
実装前または実装中にブラウザで確認すべきもの:

1. **YouTube `before:`/`after:`** — 動作報告はあるが非公式。任意期間指定の生命線なので要実機確認
2. **YouTube 引用符の厳密さ** — verbatim sp併用の効果込みで確認
3. **X `min_faves:`/`filter:blue_verified`** — 公式フォーム削除済みの演算子が現在も効くか
4. **Bluesky `-`除外** — API実測は済んでいるがWeb UI(bsky.app/search)での動作確認
5. **Bluesky `tab=latest`** — コードから確認済みだが実機で挙動確認
6. **note ANDの厳密性** — 複数語で全語含有になるかサンプル確認
7. **YouTube Shortsフィルタのsp値** — 必要なら実機でフィルタ選択→URLコピーで採取
