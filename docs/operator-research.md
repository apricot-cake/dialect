...
「このどれかを含む」箱(スコープ限定OR・入れ子なし・OR演算子を表に出さない案)を将来実装するかどうかの判断材料として、OR撤去前の実測記録([[additive-narrowing-principle]]参照、OR自体は現在Dialectの思想として全廃済み)を全20サイトぶんGUI操作で裏取りした。**本節は調査のみ、機能実装はしていない**。
...
| YouTube | `a \| b` (パイプ、`%7C`) | 未確認(pipeの複数使用は今回未検証) | 未確認 |
...
### スコープ限定OR実装時のper-siteシリアライズ要件(判断材料)
...
2. **パイプ派(YouTube)**: キーワード群を`w1|w2|...`として結合。複数グループの扱い(例えば通常キーワードとの併用時の優先順位)は未検証のため、実装時に追加調査が必要
