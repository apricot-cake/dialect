...
| Bluesky | doc(公式検索ブログ=唯一) + GUI操作 | ✅ 済 | 将来のフィルタ有効化に注意 | 2026-07-09 ログイン済みで検索フォーム(言語ドロップダウン=`&lang=`)・トップ/最新タブを実操作採取。構造化フィルタ(`author=`/`media=`)はまだ未露出(ゲート中)=有効化されたら再採取が要る |
| YouTube | doc(公式ヘルプ) + URL叩き(sp protobuf) + GUI操作 | ✅ 済 | — | 2026-07-07/08 フィルタパネルの「特徴」10項目・タイプ・並び順を実操作採取(未ログイン) |
| note | GUI操作 + doc(公式) | ✅ 済 | — | 2026-07-09 検索フォーム(並び順チップ人気/急上昇/新着=`sort=popular/hot/new`・有料フィルタ=`context=note_for_sale`)を実操作採取(未ログイン)。フォーム操作でのオプション網羅を確認 |
| niconico(動画) | doc(snapshot API) + GUI操作 | ✅ 済 | — | 2026-07-08 ジャンル17値、2026-07-09 ログイン済みで並び順ドロップダウン(9値)・検索フィルタモーダル(動画種別`kind=`/再生時間`l_range=`/任意期間`start=/end=`)を実操作採取。並び順の現行パラメータは`sort=<名前>&order=desc`形式(旧`sort=h`/`sort=f&order=d`も有効なエイリアス) |
...
**監査候補は2026-07-09に全件解消**: 旧・監査候補だった Bluesky / note / niconico動画 の3件を、実際に検索フォーム・並び順・フィルタパネルをGUI操作して採取し直した(詳細は下記「監査候補3件のGUI再点検(2026-07-09)」節)。この再点検で複数の新オプション(niconico 動画種別`kind=`・note 有料`context=note_for_sale`・Bluesky 言語`&lang=`パラメータ化・niconico並び順の追加値)を発見。3件とも「表層で止めない」水準(フォーム/フィルタ操作での網羅採取)を満たした。**唯一の残り監視点**: Blueskyの構造化フィルタ(`author=`/`media=`)はまだゲート中で未露出=有効化されたら再採取が要る(候補ではなく将来の変化点として台帳に注記)。
